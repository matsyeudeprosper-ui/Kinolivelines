"""harvest_bigbrick_m15_bot.py - the M15 BIG-BRICK-GATED harvest, DEMO account.

DEPLOYED 2026-08-09 ON THE USER'S EXPLICIT DECISION, with the backtest
...
def open_one(direction, why):
    ...
    req = {"action": mt5.TRADE_ACTION_DEAL, "symbol": SYMBOL, "volume": LOTS,
           ...
           "deviation": 30, "magic": MAGIC, "comment": "KL-bb15",
...
def close_all(ps, why):
    ...
    for p in ps:
        ...
        req = {"action": mt5.TRADE_ACTION_DEAL, "position": p.ticket, "symbol": SYMBOL,
               ...
               "price": want,
               "deviation": 30, "magic": MAGIC, "comment": "KL-bb15-close"}
        _t0 = time.perf_counter()
        r = mt5.order_send(req)
        _lat = round((time.perf_counter() - _t0) * 1000, 1)
        ok = r is not None and r.retcode in (mt5.TRADE_RETCODE_DONE, mt5.TRADE_RETCODE_PLACED)
                ...
                if age <= FRESH_MIN:
                    ...
                    if not ps:
                        ...
                        if bd != rev["dir"] or not fresh:
                            ...
                            st["cycle_tickets"] = []
                            tk = open_one(rev["dir"], "new cycle (big-brick pass)")
                            if tk:
