...
SYMBOL      = "BTCUSDm"
LOTS        = 0.01
MAGIC       = 770411            # demo copy of the harvest-live strategy, moved off
                                 # live 2026-08-14 to make room for tail_guard_m1_live_bot.py.
                                 # 770404/5/8/9/10 are the other demo bots, 770407 is live.
BRICK       = 50.0
REVERSAL    = 2
TP_BRICKS   = 5
...
LOG   = os.path.join(HERE, "harvest_live_demo.log")
ALIVE = os.path.join(HERE, "harvest_live_demo_alive.json")
STATE = os.path.join(HERE, "harvest_live_demo_state.json")
...
def check_account():
    """DEMO account 436771046 only, verified on EVERY loop rather than once at
    startup - a terminal can be re-logged into a different account while this is
    running. This is the demo copy - the original moved to live 134499778."""
    a = mt5.account_info()
    if a is None:
        return None
    if a.login != LOGIN:
...
def main():
    acc = connect()
    say("=" * 70)
    say(f"HARVEST BOT UP - DEMO (moved off live 2026-08-14 to make room for "
        f"tail_guard_m1_live_bot.py) - account {acc.login} {acc.server} "
