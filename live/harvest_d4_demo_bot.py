"""harvest_d4_demo_bot.py - the D4-GATED arm of an A/B test, DEMO account.

THIS IS A FORWARD TEST, deployed 2026-08-08 at the user's request. It is a
byte-for-byte copy of renko_recovery_bot.py (magic 770405, the CONTROL) with
exactly ONE difference: a NEW CYCLE is only opened when the D4 structure test
passes - the last two CONFIRMED 5-bar swing lows rising for a BUY, the last
two confirmed swing highs falling for a SELL (strict fractals, pivot i known
only from bar i+2, same no-lookahead rule as the backtest). Recovery adds are
UNCHANGED. Compare 770409 ONLY to 770405 - same account, same signals.

Why it earns a demo slot: in SPEC_HHLL_VARIANTS validation D4 beat the
rate-matched random control on M1 and M5 (5/6 anchors both) and missed the
beats-A0 bar by $1 on M1 (+24.22 vs 2SE 25.45). The backtest cannot settle a
margin that thin; only forward data can. This bot is the arbiter. If the gap
between 770409 and 770405 is not clearly positive after a fair sample of
cycles, D4 closes for good.
...
SYMBOL      = "BTCUSDm"
LOTS        = 0.01
MAGIC       = 770409            # D4-gated arm; 770405 is the ungated control
BRICK       = 50.0
REVERSAL    = 2
TP_BRICKS   = 5
...
LOG   = os.path.join(HERE, "harvest_d4_demo.log")
ALIVE = os.path.join(HERE, "harvest_d4_demo_alive.json")
STATE = os.path.join(HERE, "harvest_d4_demo_state.json")
...
def d4_pass(closed, direction, sig_time):
    """THE ONE DIFFERENCE from the control bot. SPEC_HHLL_VARIANTS D4: for a
    BUY the last two CONFIRMED 5-bar swing lows must be rising; for a SELL the
    last two confirmed swing highs must be falling. Strict fractals (unique
    extreme in the 5-bar window); pivot i is known only from bar i+2 - the
    same no-lookahead rule as the backtest and the observer's d4_pass column.
    Fewer than two confirmed pivots -> False (block), matching the backtest
    mask; with weeks of M1 history this cannot happen in practice."""
    tm = closed["time"]
    j = len(tm) - 1
    while j >= 0 and int(tm[j]) > sig_time:
        j -= 1
    if j < 4:
        return False
    arr = closed["low"].astype(float) if direction == 1 else closed["high"].astype(float)
    vals = []
    i = j - 2
    stop = max(2, j - 3000)
    ...
    return (v1 > v0) if direction == 1 else (v1 < v0)
...
def open_one(direction, why):
    ...
    req = {"action": mt5.TRADE_ACTION_DEAL, "symbol": SYMBOL, "volume": LOTS,
           ...
           "deviation": 30, "magic": MAGIC, "comment": "KL-d4",
...
def close_all(ps, why):
    ...
    for p in ps:
        ...
        req = {"action": mt5.TRADE_ACTION_DEAL, "position": p.ticket, "symbol": SYMBOL,
               ...
               "price": want,
               "deviation": 30, "magic": MAGIC, "comment": "KL-d4-close"}
        _t0 = time.perf_counter()
        r = mt5.order_send(req)
        _lat = round((time.perf_counter() - _t0) * 1000, 1)
        ok = r is not None and r.retcode in (mt5.TRADE_RETCODE_DONE, mt5.TRADE_RETCODE_PLACED)
        ...
        try:
            ...
            if rev and rev["time"] > st.get("last_brick", 0):
                ...
                if age <= FRESH_MIN:
                    ps = mine()
                    if not ps:
                        # THE GATE - the only behavioural difference from the
                        # 770405 control. New cycles only on a D4 pass;
                        # recovery adds below are untouched.
                        if not d4_pass(rates[:-1], rev["dir"], rev["time"]):
                            side = "BUY" if rev["dir"] == 1 else "SELL"
                            say(f"skip new cycle: D4 structure disagrees with {side}")
                            rec_event("d4_skip", side=side, signal_time=rev["time"])
                            st["last_brick"] = rev["time"]
                        else:
                            # a new cycle starts with an empty ticket list, so
                            # its P&L begins at exactly zero
                            st["cycle_tickets"] = []
                            tk = open_one(rev["dir"], "new cycle (D4 pass)")
                            if tk:
