...
orders. Events -> owl_sstop_paper.log."""
import json
import os
import time
    ...
    if pc > po and cc > co and cc > ph:
        ...
        if cc > co:
            ...
            up.update(pending=up["peak"], plow=cl, leg=False,
                      pt=t, retest=None)
            say(f"SSTOP: peak {up['pending']:.2f} official -> pending "
    ...
    if pc < po and cc < co and cc < pl:
        ...
        if cc < co:
            ...
            dn.update(pending=dn["dip"], phigh=ch, leg=False,
                      pt=t, retest=None)
            say(f"SSTOP: dip {dn['pending']:.2f} official -> pending "
...
def main():
    if not mt5.initialize(path=TERM):
        say(f"SSTOP ERROR: mt5 init failed {mt5.last_error()}")
        return
    mt5.symbol_select(SYMBOL, True)
    st = load()
    say("SSTOP paper tracker STARTED (live detector + adaptive TP, "
        "virtual only)")
    while True:
        time.sleep(5)
        ...
        if p is not None:
            ...
            if not p["locked"] and fav >= 0.40 * prize:
                ...
                say(f"SSTOP 40% LOCK (virtual): wall moved to "
                    f"{p['sl']:.2f}")
                save(st)
            ...
            if hit_tp or hit_sl:
                ...
                say(f"SSTOP EXIT (virtual): {kind} {pnl:+.2f} after "
        ...
        if sig is not None:
            ...
            if dist < MIN_WALL or dist * LOT > PAGE_MAXR:
                say(f"SSTOP skipped: wall {dist:.0f}pts")
            else:
                tpd = dist - min(1.0, 0.25 * dist * LOT) / LOT
                tpd = min(tpd, PAGE_TGT / LOT)
                tpd = tpd * 0.25
                # SMART STOP trial: risk capped at 25pts even when
                # the structural wall is farther (backtest -1.3..+2
                # over 69d under all tie assumptions)
                slw = wall if dist <= 25.0 else (e - d * 25.0)
                st["pos"] = {"d": d, "e": e, "sl": round(slw, 2),
                             "tp": round(e + d * tpd, 2),
                             "locked": False, "t0": time.time()}
                say(f"SSTOP ENTRY (virtual): "
                    f"{'BUY' if d == 1 else 'SELL'} {LOT} @ {e:.2f} "
                    f"SL {st['pos']['sl']:.2f} (cap 25pts) TP "
                    f"{st['pos']['tp']:.2f} (risk "
                    f"${min(dist, 25.0) * LOT:.2f}, prize "
                    f"${tpd * LOT:.2f})")
        save(st)
