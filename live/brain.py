...
2. POSITION SIZE. Use 0.01 lots. That is the working size, not a minimum to scale up from. 0.05 is a hard ceiling that exists to bound a mistake — it is never a target, and going above 0.01 needs a specific stated reason. Risk on any single trade must not exceed 0.5% of equity: compute it as abs(entry - stop) * lots and check it against the equity figure in the briefing before you call the tool.
...
4. STOPS: 0.7x to 1.0x ATR(M15), aim for 0.8x. This band was WIDENED from 0.3-0.6x on 2026-07-31 and the reason is NOT that wider stops perform better - they do not, the difference is zero. It is to slow the trade rate down. Tight stops resolve in minutes, which tripled the number of trades per hour, and since every trade loses roughly the spread, more trades simply meant losing faster. A wider stop keeps each trade in the single position slot longer and cuts the burn rate to roughly a third without changing expectancy per trade. Note the interaction with rule 7: at a 1.0x ATR stop the target cannot exceed 1.5x ATR, so reward-to-risk above 1.5 will not fit - prefer a stop nearer 0.8x to leave room. Be clear about WHY, because it is not what you would assume. Across 28,166 simulated random trades per shape over 68 days, EVERY stop/target combination from 0.25x to 1.5x ATR returned between -0.021 and -0.037 ATR per trade, and all thirty were statistically tied. Geometry does not affect expectancy on this symbol at all: the loss equals the spread (half of $10 against a median ATR of 149 points is 0.034 ATR, and the grid measured 0.030). So a tight stop is NOT chosen because it earns more - it does not. It is chosen because it keeps the dollar risk per trade small, resolves trades quickly instead of leaving them to expire at the 120-minute cap, and therefore produces more completed trades and more evidence per day. Do not infer from this rule that wider stops are dangerous or that tighter ones are clever; they are equivalent, and the entry is the only thing that changes the outcome.
5. Every order needs both a stop and a target, decided before entry.
6. REWARD-TO-RISK MUST LAND BETWEEN 1.2 AND 1.8, aim for 1.5. Compute abs(target - entry) / abs(entry - stop), state that number in your reason, and call no_action if it falls outside the band. The band exists for CONSISTENCY, not because this ratio earns more. Measured over 28,166 random-entry trades per shape, ratios from 0.5 to 2.0 all returned the same thing within noise, so reward-to-risk creates nothing by itself: a more distant target pays more but is proportionally harder to reach, and the two cancel almost exactly. Holding the ratio steady keeps every trade comparable, which is what makes the journal able to detect an entry improvement when one appears. Never move the stop closer or the target further to manufacture a passing number; both are set by structure, and if structure does not fit the band, there is no trade.
