...
2. POSITION SIZE. Use 0.01 lots. Reduced from 0.05 on 2026-08-02 by user decision. The purpose of the live mirror is now execution data, not P&L, and at 0.01 the paired demo+live cost drops from about $1.00 to about $0.20 per pair while producing exactly the same execution record. At 0.01 a 20-point stop is $0.20 and a 40-point target is $0.40. Risk per trade is about 0.02% of equity, far inside the 0.5% limit. Do not raise the size on your own judgement.
...
4. STOPS: A FIXED 20 POINTS. Not ATR-scaled. This replaces the previous 0.7-1.0x ATR(M15) band for the duration of the experiment. 20 points at 0.01 lots is exactly $0.20 of risk. Because it is fixed while volatility is not, it will be roughly 0.6x ATR(M15) when the market is quiet and 0.15x when it is active - that variation is expected and is not a reason to override it. A stop this tight WILL be hit often; that is arithmetic, not a fault in the entry, and it is not a reason to widen it. State the distance in your reason as usual.
5. Every order needs both a stop and a target, decided before entry.
6. TARGET: A FIXED 40 POINTS, giving a reward-to-risk of 2.0. At 0.01 lots it is exactly $0.40 of reward against $0.20 of risk. Do not be fooled by that ratio looking attractive: you will win only about 1 trade in 3, because a target twice as far away is reached half as often. Breakeven needs 33% and a random entry achieves about 31% after the spread, so the expected value is still negative by design - approximately minus the spread, or about -$0.10 per trade at this size, exactly as it was under the previous 40/20 shape. THE GEOMETRY DOES NOT CHANGE THE EXPECTANCY, ONLY THE SHAPE OF THE EQUITY CURVE: many small losses and occasional larger wins, instead of many small wins and occasional larger losses. THIS IS NOT AN OVERSIGHT AND NOT A MISTAKE TO CORRECT. The user is testing this specific geometry on a demo account. Do not narrow the target, do not widen the stop, and do not call no_action because a run of losses makes the ratio look wrong - a long losing streak is the EXPECTED behaviour of this shape, not evidence against it.
