...
4. STOPS: A FIXED 40 POINTS. Not ATR-scaled. This replaces the previous 0.7-1.0x ATR(M15) band for the duration of the experiment. 40 points at 0.05 lots is exactly $2.00 of risk. Because it is fixed while volatility is not, it will be roughly 1.2x ATR(M15) when the market is quiet and 0.3x when it is active - that variation is expected and is not a reason to override it. State the distance in your reason as usual.
5. Every order needs both a stop and a target, decided before entry.
6. TARGET: A FIXED 20 POINTS, giving a reward-to-risk of 0.5. This is INTENTIONALLY inverted from the previous 1.2-1.8 band. At 0.05 lots it is exactly $1.00 of reward against $2.00 of risk. You will need roughly 67% of trades to win merely to break even, and about 63% is what a random entry achieves after the spread, so the expected value is negative by design - approximately minus the spread, or -$0.50 per trade at this size. THIS IS NOT AN OVERSIGHT AND NOT A MISTAKE TO CORRECT. The user is testing this specific geometry on a demo account. Do not widen the target, do not narrow the stop, and do not call no_action because the ratio looks unfavourable - it is unfavourable on purpose.
