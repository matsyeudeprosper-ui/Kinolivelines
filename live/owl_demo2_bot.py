...
TERMINAL = r"C:\Projects\MT5-KinoliveTrader-Session2\terminal64.exe"
SYMBOL = "BTCUSDm"
TP_USD = 1.0               # fallback only; entries carry structural TP
...
AUTO_ENTRY = True          # 2026-08-22 user: "implement my entry" - the Owl
...
AUTO_LOTS = 0.02
AUTO_MAX_STACK = 1         # one position at a time
AUTO_MIN_BAL = 50.0
DIP_GATE = False    # DEMO2: no dip gate            # 2026-08-23 user: dip-in-trend gate on auto entries.
...
SPLIT_TP = False    # DEMO2: single 0.02            # 2026-08-23 user: split entry - 2x0.01 instead of
...
LONDON_OFF = False  # DEMO2: all sessions          # 2026-08-24 user-approved: NO new auto entries
...
SWEEP_ENTRY = False # DEMO2: no Croc here         # 2026-08-24 user: Range Sweep V1 at HALF size.
...
LOG = os.path.join(DIR, "owl_demo2.log")
ALIVE = os.path.join(DIR, "owl_demo2_alive.json")
STATE = os.path.join(DIR, "owl_demo2_state.json")
JOURNAL = os.path.join(DIR, "owl_demo2_journal.csv")
MARKETLOG = os.path.join(DIR, "owl_demo2_market_log.csv")
...
def reassign(st, manual, tick):
    ...
    for side in (mt5.POSITION_TYPE_BUY, mt5.POSITION_TYPE_SELL):
        ...
        winners = unassigned              # everyone keeps the $1 TP
...
def main():
    ...
    fib_touched = set()    # FIB-GREEN: levels touched this hour
    while True:
        try:
            if not connect():
                time.sleep(10)
            ...
            if b3 is not None and len(b3) >= 3:
                ...
                if cbt != sh_last_bar:
                    ...
                    if hr != sh_hour:
                        ...
                        fib_touched = set()
                    al = st.get("aligned", 0)
                    def _auto_fire(direction, tp_price=None):
                        if not AUTO_ENTRY:
                            return
                        if escape_active:
                        ...
                        if r is not None and r.retcode == mt5.TRADE_RETCODE_DONE:
                            say(f"AUTO ENTRY: fired -> "
                                f"{'BUY' if direction == 1 else 'SELL'} {AUTO_LOTS} opened")
                            if tp_price is not None:
                                rr = mt5.order_send({"action": mt5.TRADE_ACTION_SLTP,
                                                     "position": r.order, "symbol": SYMBOL,
                                                     "sl": 0.0, "tp": round(tp_price, 2)})
                                if rr is not None and rr.retcode == mt5.TRADE_RETCODE_DONE:
                                    st["user_owned"].append(str(r.order))
                                    save_state(st)
                                    say(f"STRUCT TP set: ticket {r.order} -> {round(tp_price, 2)}")
                                else:
                                    say(f"STRUCT TP FAILED ticket {r.order} "
                                        f"retcode={rr.retcode if rr else None} - $1 fallback governs")
                        else:
                            say(f"AUTO ENTRY FAILED retcode={r.retcode if r else None}")
                    # FIB-GREEN (validated 2026-08-25): green light (al!=0)
                    # + H4/H1 candle compass direction + fib touch->reclaim
                    if al != 0 and not sh_fired:
                        h1cd = mt5.copy_rates_from_pos(SYMBOL, mt5.TIMEFRAME_H1, 1, 1)
                        h4cd = mt5.copy_rates_from_pos(SYMBOL, mt5.TIMEFRAME_H4, 1, 1)
                        w1cd = mt5.copy_rates_from_pos(SYMBOL, mt5.TIMEFRAME_W1, 1, 1)
                        if (h1cd is not None and len(h1cd)
                                and h4cd is not None and len(h4cd)
                                and w1cd is not None and len(w1cd)):
                            _c1 = h1cd[0]
                            _c4 = h4cd[0]
                            _cw = w1cd[0]
                            dcw = 1 if _cw["close"] > _cw["open"] else (
                                -1 if _cw["close"] < _cw["open"] else 0)
                            dc1 = 1 if _c1["close"] > _c1["open"] else (
                                -1 if _c1["close"] < _c1["open"] else 0)
                            dc4 = 1 if _c4["close"] > _c4["open"] else (
                                -1 if _c4["close"] < _c4["open"] else 0)
                            W = float(_c1["high"] - _c1["low"])
                            if dc1 != 0 and dc1 == dc4 and dc1 == dcw and W > 20:
                                for fk, ff in enumerate((0.5,)):
                                    if dc1 == 1:
                                        lvl = float(_c1["high"]) - ff * W
                                        if cb["low"] <= lvl:
                                            fib_touched.add(fk)
                                        if (fk in fib_touched and cb["close"] > lvl
                                                and cb["close"] > cb["open"]):
                                            sh_fired = True
                                            _tgt = float(_c1["high"])
                                            if _tgt - cb["close"] <= 2.0:
                                                break
                                            say(f"MID-RECLAIM: 50% reclaimed -> BUY "
                                                f"(lvl {lvl:.2f}, target {_tgt:.2f})")
                                            _auto_fire(1, tp_price=_tgt)
                                            break
                                    else:
                                        lvl = float(_c1["low"]) + ff * W
                                        if cb["high"] >= lvl:
                                            fib_touched.add(fk)
                                        if (fk in fib_touched and cb["close"] < lvl
                                                and cb["close"] < cb["open"]):
                                            sh_fired = True
                                            _tgt = float(_c1["low"])
                                            if cb["close"] - _tgt <= 2.0:
                                                break
                                            say(f"MID-RECLAIM: 50% rejected -> SELL "
                                                f"(lvl {lvl:.2f}, target {_tgt:.2f})")
                                            _auto_fire(-1, tp_price=_tgt)
                                            break
            ...
            with open(ALIVE, "w") as f:
                json.dump({
                    ...
                    "bot": "OWL-demo2-MIDWEEKLY (DEMO 476604490)",
