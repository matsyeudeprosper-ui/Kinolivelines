...
Events -> owl_userdet_slots.log, totals -> owl_userdet_slots.json.
...
LOG = os.path.join(DIR, "owl_userdet_slots.log")
STATE = os.path.join(DIR, "owl_userdet_slots.json")
SYMBOL = "BTCUSDm"
    ...
    if not up.get("pending") and not up.get("retest"):
        if up.get("hh") is None or ch > up["hh"]:
            ...
            up.update(pending=up["hh"], plow=cl, pt=t, retest=None,
                      hh=None, hhlow=None)
            say(f"UDET2: peak {up['pending']:.2f} official (close "
        ...
        if t - up["pt"] > 21600:
            ...
            if cc - up["pending"] > max(20.0, 0.35 * wd):
                up.update(retest=up["pending"], rt_plow=up["plow"],
                          rt_t=t, pending=None)
                say(f"UDET2: BUY confirmation ran away - retest armed "
                    f"at {up['retest']:.2f}")
            else:
                sig = (1, up["plow"])
                up["pending"] = None
    ...
    if not dn.get("pending") and not dn.get("retest"):
        if dn.get("ll") is None or cl < dn["ll"]:
            ...
            dn.update(pending=dn["ll"], phigh=ch, pt=t, retest=None,
                      ll=None, llhigh=None)
            say(f"UDET2: dip {dn['pending']:.2f} official (close "
        ...
        if t - dn["pt"] > 21600:
            ...
            if dn["pending"] - cc > max(20.0, 0.35 * wd):
                dn.update(retest=dn["pending"], rt_phigh=dn["phigh"],
                          rt_t=t, pending=None)
                say(f"UDET2: SELL confirmation ran away - retest armed "
                    f"at {dn['retest']:.2f}")
            else:
                sig = (-1, dn["phigh"])
                dn["pending"] = None
...
def main():
    if not mt5.initialize(path=TERM):
        say(f"UDET2 ERROR: mt5 init failed {mt5.last_error()}")
        return
    mt5.symbol_select(SYMBOL, True)
    st = load()
    say("UDET2 paper tracker STARTED (virtual only, no broker orders)")
    while True:
        time.sleep(5)
        ...
        if p is not None:
            ...
            if not p["locked"] and fav >= 0.40 * prize:
                ...
                say(f"UDET2 40% LOCK (virtual): wall moved to "
                    f"{p['sl']:.2f}")
                save(st)
            ...
            if hit_tp or hit_sl:
                ...
                say(f"UDET2 EXIT (virtual): {kind} {pnl:+.2f} after "
        ...
        sig = det_step(st, b)
        if sig is not None:
            # SLOT LIMIT (user 2026-09-06): max 2 trades/hour - one in
            # min 00-29, one in min 30-59; a used slot skips signals
            _nowu = datetime.now(timezone.utc)
            _slot = _nowu.hour * 2 + (0 if _nowu.minute < 30 else 1)
            _skey = _nowu.strftime(chr(37)+'Y'+chr(37)+'m'+chr(37)+'d') + '-' + str(_slot)
            if st.get('last_slot') == _skey:
                if st.get('last_slot_said') != _skey:
                    st['last_slot_said'] = _skey
                    say('UDET2 slot already used - signal skipped '
                        '(next slot at ' + ('30' if _nowu.minute < 30 else '00') + 'min)')
                save(st)
                continue
            st['pending_slot'] = _skey
        if sig is not None:
            d, wall = sig
            e = tick.ask if d == 1 else tick.bid
            dist = abs(e - wall)
            if dist < MIN_WALL:
                say(f"UDET2 skipped: wall {dist:.0f}pts too close")
            elif dist * LOT > PAGE_MAXR:
                say(f"UDET2 skipped: wall {dist:.0f}pts would risk "
                    f"${dist * LOT:.2f} > ${PAGE_MAXR:.2f}")
            else:
                tpd = dist - min(1.0, 0.25 * dist * LOT) / LOT
                tpd = min(tpd, PAGE_TGT / LOT) * TPF
                st["last_slot"] = st.get("pending_slot"); st["pos"] = {"d": d, "e": e, "sl": round(wall, 2),
                             "tp": round(e + d * tpd, 2),
                             "locked": False, "t0": time.time()}
                say(f"UDET2 ENTRY (virtual): "
