...
print(f'CROC + HOUR-FLAT (live current behavior): boundary closes everything | {NMIN}min | START {START_BAL}', flush=True)
cur_flat_hour = -1
...
for i in range(N):
    _hid_now = tm[i] // 3600
    if _hid_now != cur_flat_hour:
        cur_flat_hour = _hid_now
        for p in positions[:]:
