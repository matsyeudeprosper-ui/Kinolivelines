"""Rolling/relative volatility filter: skip a signal if current ATR14 is
in the bottom 25% of its OWN trailing 30-day window (recomputed
continuously, not calibrated once from a fixed past period). Adapts to
regime shifts instead of assuming the past defines the future.
"""
import numpy as np
...
ROLL_WINDOW_H = 720  # 30 days of H1 bars
ATR_FLOOR_PCTILE = 25
...
def rolling_pctile(atr, offset, j):
    gj = offset + j
    lo = max(0, gj - ROLL_WINDOW_H + 1)
    window = atr[lo:gj+1]
    if len(window) < 24:
        return 100.0  # not enough history yet, don't filter
    return 100.0 * np.mean(window <= atr[gj])


def run(o,h,l,c,tm,atr,offset,N,sigs,SL_PTS,use_filter):
    ...
    for j in range(N):
        ...
        if j in sigs and j+1 < N and not in_pos:
            skip = use_filter and rolling_pctile(atr, offset, j) < ATR_FLOOR_PCTILE
            if skip:
                filtered_out += 1
            else:
...
print(f"Rolling filter: skip if ATR14 is below the {ATR_FLOOR_PCTILE}th percentile of its OWN trailing {ROLL_WINDOW_H}h (30d) window\n")
print(f"{'anchor':>7} {'test period':>23} {'trades':>7} {'tr/day':>7} {'win%':>6} {'meanDur':>9} {'medDur':>8} {'maxDur':>9} {'ended':>10} {'lowest':>9}")
results = []
for i in range(1, NSEG):
    ...
    z = run(ot,ht,lt,ct,tmt,atr14,test_start,Nt,sigt,SL_PTS,use_filter=True)
    d0 = datetime.utcfromtimestamp(tmt[0]).strftime("%Y-%m-%d")
    d1 = datetime.utcfromtimestamp(tmt[-1]).strftime("%Y-%m-%d")
    results.append(z)
    if z["dead"]:
        print(f"seg {i:>3}   {d0} to {d1}   *** DIED ***  (filtered {z.get('filtered_out','?')})")
    else:
        print(f"seg {i:>3}   {d0} to {d1}  {z['trades']:>7} {z['trades_per_day']:>6.2f} "
              f"{z['winrate']:>5.1f}% {z['mean_dur']:>8.1f}h {z['median_dur']:>7.1f}h {z['max_dur']:>8.0f}h "
              f"${z['end']:>9,.2f} ${z['lo']:>8,.2f}  (filtered {z['filtered_out']})")
...
print(f"\n=== FOR COMPARISON ===")
print(f"cap=1 no filter:        seg1 $1031 | seg2 $754(loss) | seg3 $1074 | seg4 $1258 | seg5 $1302 | 0/5 died")
print(f"cap=1 fixed ATR filter: seg1 $1052 | seg2 $749(loss) | seg3 $1023 | seg4 $1128 | seg5 $1299 | 0/5 died")
