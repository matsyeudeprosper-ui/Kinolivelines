...
print(f'TOUCH-FIRST: H1 color only; prev-H1 extreme must be TOUCHED first, then recipe; TP = retest of that extreme | START {START_BAL}', flush=True)
pending_target = None
hr_hi = -1e18
hr_lo = 1e18
hr_id_track = -1
...
for i in range(N):
    ...
    if h1dir == 0:
        continue                      # doji hour: no direction
    ...
    if h1dir == 1 and _cond_hi < _h1[1]:
        continue                      # momentum proof required: prev high must be touched first
