...
FIB_ONE = float(_sys.argv[5]) if len(_sys.argv) > 5 else 0.382
print(f'FIB ONE-SHOT: single entry at first touch of the {FIB_ONE} retracement, group exit +${GROUP_TP} | START {START_BAL}', flush=True)
fib_hour = -1
fib_used = set()
...
for i in range(N):
    ...
    for _k, _f in enumerate(FIBS):
        if _f != FIB_ONE:
            continue                  # one-shot: only the chosen level
        if _k in fib_used:
            continue
        if positions:
