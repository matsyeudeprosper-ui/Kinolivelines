...
print(f'SPIKY: 4xATR spike-wick reclaim | cost {RT}pts | {NMIN}min | START {START_BAL}', flush=True)
...
_tr = h - l
_atr14 = np.convolve(_tr, np.ones(14) / 14.0, mode='full')[:len(_tr)]
_atr14[:14] = _atr14[14]
...
for i in range(N):
    ...
    if cur_hour == last_entry_hour or i < 1 or i + 1 >= N:
        continue
    rng = h[i] - l[i]
    if rng < 4.0 * _atr14[i - 1] or rng <= 30:
        continue                      # not a spike bar
    ...
    if lower_wick >= 0.6 * rng and c[i] > o[i]:
        pending = 1                   # panic spike down, rejected, green close
        last_entry_hour = cur_hour
    elif upper_wick >= 0.6 * rng and c[i] < o[i]:
        pending = -1                  # blow-off spike up, rejected, red close
        last_entry_hour = cur_hour
