...
G4H = float(_sys.argv[3]) if len(_sys.argv) > 3 else 99.0
print(f'FINAL COMBO: twin-breakout + split TP + {NMIN}min + calm {G4H}% | START {START_BAL}', flush=True)
h1_prev1 = None
h1_prev2 = None
...
for i in range(N):
    ...
    while maxq and maxq[0][0] <= i - 240: maxq.popleft()
    while minq and minq[0][0] <= i - 240: minq.popleft()
    while maxq and maxq[-1][1] <= h[i]: maxq.pop()
    maxq.append((i, h[i]))
    while minq and minq[-1][1] >= l[i]: minq.pop()
    minq.append((i, l[i]))
    range4h = 100.0 * (maxq[0][1] - minq[0][1]) / c[i] if c[i] else 99.0
    ...
    if cur_hour != rcp_hour:
        ...
        rcp_pulled = False
    if range4h >= G4H:
        continue                      # calm-pocket gate
    if h1_prev1 is None or h1_prev2 is None:
        continue
    if mode == 1 and not (h1_prev1[3] > h1_prev1[0] and h1_prev2[3] > h1_prev2[0] and h1_prev1[3] > h1_prev2[1]):
        continue                      # twin-breakout for buys
    if mode == -1 and not (h1_prev1[3] < h1_prev1[0] and h1_prev2[3] < h1_prev2[0] and h1_prev1[3] < h1_prev2[2]):
        continue                      # twin-breakout for sells
