...
def run_full(o,h,l,c,tm,N,sigs,trig_f,cap_f,risky_sl_pct):
    """Every signal still gets traded (no skipping). 'Confirmed' = the latest
    closed kept breakout-chain M5 bar agrees with the trade direction (BUY
    wants dir=1, SELL wants dir=-1) - keeps the normal 40% SL. 'Risky' =
    disagreement (or no data yet) - gets a tightened default SL width."""
    ...
    for j in range(N):
        ...
        if j in sigs and j+1<N and not in_pos:
            ...
            base_pct = REL_SL_PCT if confirmed else risky_sl_pct
...
print("\nrunning baseline (Ratchet only, all trades at normal 40% SL)...")
...
print("\n=== sweep: tighten SL for 'unconfirmed' trades only, all trades still taken ===")
...
print("\n=== fine sweep around 35% ===")
for risky_pct in [0.33,0.34,0.345,0.35,0.355,0.36,0.37,0.38]:
    t, n = run_full(o_f,h_f,l_f,c_f,tm_f,N,sigs,0.30,1.00,risky_pct)
    l = sum(1 for t_ in t if t_[3])
    print(f"{100*risky_pct:>7.1f}%   {len(t):>6}   {l:>6}   ${n:>10,.2f}   ${n-n0:>+10,.2f}")

print("\n=== fine sweep around 15% ===")
for risky_pct in [0.11,0.12,0.13,0.14,0.145,0.15,0.155,0.16,0.17,0.18]:
    t, n = run_full(o_f,h_f,l_f,c_f,tm_f,N,sigs,0.30,1.00,risky_pct)
    l = sum(1 for t_ in t if t_[3])
    print(f"{100*risky_pct:>7.1f}%   {len(t):>6}   {l:>6}   ${n:>10,.2f}   ${n-n0:>+10,.2f}")
