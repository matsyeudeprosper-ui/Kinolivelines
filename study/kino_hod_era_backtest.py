...
print(f'START_BAL {START_BAL}  HOUR-FLAT variant', flush=True)
last_flat_hour = -1
...
# per-hour-of-day P&L breakdown (server time)
import collections
hod_pnl = collections.defaultdict(float)
hod_n = collections.defaultdict(int)
for t in trades:
    if t[2] in ('tp', 'be', 'hour_clean', 'open_end', 'hour_flat'):
        hh = (int(t[0]) // 3600) % 24
        hod_pnl[hh] += t[1]
        hod_n[hh] += 1
from datetime import datetime as _dt
era_bounds = [(_dt(2020,8,16).timestamp(), _dt(2022,8,16).timestamp()),
              (_dt(2022,8,16).timestamp(), _dt(2024,8,16).timestamp()),
              (_dt(2024,8,16).timestamp(), _dt(2026,8,24).timestamp())]
print('ERA-STABILITY of key hours (per-trade cents in era1/era2/era3):')
for hh in [0, 12, 13, 22, 3, 4]:
    parts = []
    for d0, d1e in era_bounds:
        sel = [t[1] for t in trades if t[2] in ('tp','be','hour_clean','open_end','hour_flat')
               and d0 <= t[0] < d1e and (int(t[0]) // 3600) % 24 == hh]
        parts.append(f"{100*sum(sel)/len(sel) if sel else 0:+.1f}c(n={len(sel)})")
    print(f'  {hh:02d}:00 -> ' + ' | '.join(parts))
print('HOUR-OF-DAY table (server time): hour | trades | total pnl | per-trade')
for hh in range(24):
    n = hod_n[hh]
    s = hod_pnl[hh]
    print(f'  {hh:02d}:00 | {n:5d} | ${s:9,.2f} | {100*s/n if n else 0:+.1f}c')
