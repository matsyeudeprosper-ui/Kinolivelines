...
print(f'SESSIONS: dip+split by ENTRY session | {NMIN}min | START {START_BAL}', flush=True)
last_flat_hour = -1
...
for i in range(N):
    ...
    if cur_hour != rcp_hour:
        ...
        rcp_setup = None
        rcp_pulled = False
    if not all(k in closed for k in ('h1', 'm15', 'm5')):
        continue
    def _grn(cd): return cd[3] > cd[0]
    def _red(cd): return cd[3] < cd[0]
    if mode == 1 and not (_grn(closed['h1']) and _red(closed['m15']) and _red(closed['m5'])):
        continue
    if mode == -1 and not (_red(closed['h1']) and _grn(closed['m15']) and _grn(closed['m5'])):
        continue
