...
for i in range(N):
    ...
    if hr_now != last_flat_hour:
        if last_flat_hour != -1:
            for p in positions[:]:
                ...
                balance += pnl
                trades.append((tm[i], pnl, 'hour_flat', (tm[p['idx']] // 3600) % 24))
                positions.remove(p)
            groups.clear()
            escape_active = False
        ...
        if escape_active and mode != 0 and i > esc_start + 60:
            ...
            if h1dir == mode:
                for p in [x for x in positions if x['dir'] != mode]:
                    ...
                    trades.append((tm[i], pnl, 'escape_cut', (tm[p['idx']] // 3600) % 24))
                    positions.remove(p)
                escape_active = False
                reassign()
    ...
    for p in positions[:]:
        ...
        if hit:
            ...
            trades.append((tm[i], pnl, kind, (tm[p['idx']] // 3600) % 24))
            positions.remove(p)
            closed_any = True
    # --- hour-team cleanup ---
    cur_hour = tm[i] // 3600
...
print('ENTRY-HOUR table: hour | n | per-trade | era1/era2/era3 per-trade')
rows_all = [t for t in trades if len(t) > 3 and t[2] in ('tp','be','hour_clean','open_end','hour_flat')]
for hh in range(24):
    sel = [t for t in rows_all if t[3] == hh]
    if not sel:
        continue
    parts = []
    for d0, d1e in era_bounds:
        es = [t[1] for t in sel if d0 <= t[0] < d1e]
        parts.append(f"{100*sum(es)/len(es) if es else 0:+.0f}c")
    print(f'  {hh:02d} | {len(sel):5d} | {100*sum(t[1] for t in sel)/len(sel):+.1f}c | ' + '/'.join(parts))
