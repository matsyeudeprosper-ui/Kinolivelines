...
print(f'FOUR-CLOCK COLOR: last closed H4+H1+M15+M5 must all match direction | START {START_BAL}', flush=True)
last_flat_hour = -1
...
for i in range(N):
    ...
    if mode == 1 and not (_grn(closed['h4']) and _grn(closed['h1']) and _grn(closed['m15']) and _grn(closed['m5'])):
        continue                      # all four last candles green for buys
    if mode == -1 and not (_red(closed['h4']) and _red(closed['h1']) and _red(closed['m15']) and _red(closed['m5'])):
        continue                      # all four last candles red for sells
