...
print(f'SYMMETRIC 1:1: recipe + green light, 1/hour, TP +$3 / SL -$3, no recovery | START {START_BAL}', flush=True)
last_flat_hour = -1
...
for i in range(N):
    ...
    # --- exits: SL touches (checked FIRST - conservative) ---
    for p in positions[:]:
        if p['tp'] is None:
            continue
        _sl = p['entry'] - p['dir'] * TP_PTS
        if (l[i] <= _sl) if p['dir'] == 1 else (h[i] >= _sl):
            pnl = (_sl - p['entry']) * p['vol'] * p['dir']
            balance += pnl
            groups[p['hour']] = groups.get(p['hour'], 0.0) + pnl
            trades.append((tm[i], pnl, 'slx'))
            positions.remove(p)
    # --- exits: TP touches ---
    closed_any = False
    for p in positions[:]:
...
engine = [t for t in trades if t[2] in ('tp', 'be', 'hour_clean', 'open_end', 'hour_flat', 'slx')]
