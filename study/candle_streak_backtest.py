...
def h1_color_before(entry_hour, back):
    """back=1 -> immediately previous closed H1 candle, back=2 -> the one before that"""
    hr = entry_hour - back
...
def run_ratchet(o,h,l,c,tm,N,sigs,trig_f,cap_f):
    ...
    pos_sig_bar=None
    trades = []  # (side, m1c_1, m1c_2, h1c_1, h1c_2, hsl)
    for j in range(N):
        ...
        if in_pos:
            ...
            if htp or hsl:
                ...
                trades.append(("BUY" if pos_L else "SELL", m1c_1, m1c_2, h1c_1, h1c_2, hsl))
                in_pos=False
    return trades, bal
...
def streak_label(c1, c2):
    if c1 == 1 and c2 == 1: return "2x Green"
    if c1 == -1 and c2 == -1: return "2x Red"
    return "Mixed"

print("\n=== H1: 2-consecutive-candle streak before entry, by side ===")
h1_streak = defaultdict(lambda: [0,0])
for side, m1c1, m1c2, h1c1, h1c2, hsl in trades:
    key = (side, streak_label(h1c1, h1c2))
    h1_streak[key][1 if hsl else 0] += 1
print(f"{'Side':<5} {'H1 streak':<10} {'Wins':>7} {'Losses':>7} {'Total':>7} {'Loss %':>8}")
for key in sorted(h1_streak.keys()):
    w,l = h1_streak[key]; tot=w+l
    print(f"{key[0]:<5} {key[1]:<10} {w:>7} {l:>7} {tot:>7} {100*l/tot if tot else 0:>7.2f}%")

print("\n=== M1: 2-consecutive-candle streak before entry (signal bar + one before), by side ===")
m1_streak = defaultdict(lambda: [0,0])
for side, m1c1, m1c2, h1c1, h1c2, hsl in trades:
    key = (side, streak_label(m1c1, m1c2))
    m1_streak[key][1 if hsl else 0] += 1
print(f"{'Side':<5} {'M1 streak':<10} {'Wins':>7} {'Losses':>7} {'Total':>7} {'Loss %':>8}")
for key in sorted(m1_streak.keys()):
    w,l = m1_streak[key]; tot=w+l
    print(f"{key[0]:<5} {key[1]:<10} {w:>7} {l:>7} {tot:>7} {100*l/tot if tot else 0:>7.2f}%")

print("\n=== H1 streak, combined both sides ===")
h1_combined = defaultdict(lambda: [0,0])
for side, m1c1, m1c2, h1c1, h1c2, hsl in trades:
    h1_combined[streak_label(h1c1, h1c2)][1 if hsl else 0] += 1
for k,v in h1_combined.items():
    w,l=v; tot=w+l
    print(f"H1 {k:<10}: wins={w} losses={l} total={tot} loss%={100*l/tot if tot else 0:.2f}%")

print("\n=== M1 streak, combined both sides ===")
m1_combined = defaultdict(lambda: [0,0])
for side, m1c1, m1c2, h1c1, h1c2, hsl in trades:
    m1_combined[streak_label(m1c1, m1c2)][1 if hsl else 0] += 1
for k,v in m1_combined.items():
    w,l=v; tot=w+l
    print(f"M1 {k:<10}: wins={w} losses={l} total={tot} loss%={100*l/tot if tot else 0:.2f}%")
