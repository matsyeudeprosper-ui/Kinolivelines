...
print(f"BASELINE: net ${zb:,.2f}  losses={lb}")

print("\n=== cap fixed at 100% (simple single-cap ratchet), trigger 5-50% in 1% steps, continuous, no resets ===")
for i in range(5, 51):
    trig_f = i/100.0
    z,l = run_net(o_f,h_f,l_f,c_f,tm_f,N,sigs,trig_f,1.00)
    print(f"trig={i:>3d}%  diff={z-zb:>+9.0f}  losses={l}")
