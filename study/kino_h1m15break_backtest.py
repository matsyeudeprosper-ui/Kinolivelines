...
print(f'H1+M15 BREAKOUT: twin-breakout required on BOTH H1 and M15 | START {START_BAL}', flush=True)
h1_prev1 = None
h1_prev2 = None
m15_prev1 = None
m15_prev2 = None
...
for i in range(N):
    ...
    if h1_prev1 is None or h1_prev2 is None:
        continue
    if m15_prev1 is None or m15_prev2 is None:
        continue
    if mode == 1 and not (h1_prev1[3] > h1_prev1[0] and h1_prev2[3] > h1_prev2[0] and h1_prev1[3] > h1_prev2[1]):
        continue                      # H1 twin-breakout for buys
    if mode == 1 and not (m15_prev1[3] > m15_prev1[0] and m15_prev2[3] > m15_prev2[0] and m15_prev1[3] > m15_prev2[1]):
        continue                      # M15 twin-breakout for buys
    if mode == -1 and not (h1_prev1[3] < h1_prev1[0] and h1_prev2[3] < h1_prev2[0] and h1_prev1[3] < h1_prev2[2]):
        continue                      # H1 twin-breakout for sells
    if mode == -1 and not (m15_prev1[3] < m15_prev1[0] and m15_prev2[3] < m15_prev2[0] and m15_prev1[3] < m15_prev2[2]):
        continue                      # M15 twin-breakout for sells
