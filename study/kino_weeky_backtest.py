...
print(f'WEEKY: prev-WEEK high/low sweep+reclaim | cost {RT}pts | {NMIN}min | START {START_BAL}', flush=True)
w1_prev = None
...
for i in range(N):
    ...
    if cur_hour != rcp_hour:
        ...
        swept_lo = False
        swept_hi = False
    if len(h1_hist) < NRANGE or d1_prev is None or w1_prev is None:
        continue
