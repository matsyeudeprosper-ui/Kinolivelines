...
print(f'FIB RECLAIM: touch a prev-H1 fib then M1 CLOSES back through it -> enter | exit +${GROUP_TP} | START {START_BAL}', flush=True)
fib_touched = set()
fib_entered_hour = -1
fib_hour = -1
fib_used = set()
    ...
    for _k, _f in enumerate(FIBS):
        if h1dir == 1:
            ...
            if (_k in fib_touched and not positions and fib_entered_hour != cur_hour
                    and c[i] < _lvl and c[i] < o[i]):
                fib_entered_hour = cur_hour
                positions.append({'dir': -1, 'entry': c[i], 'vol': LOTS, 'tp_custom': None,
                                  'tp': None, 'hour': tm[i] // 3600, 'idx': i})
                groups.setdefault(tm[i] // 3600, 0.0)
                break
    max_conc = max(max_conc, len(positions))
