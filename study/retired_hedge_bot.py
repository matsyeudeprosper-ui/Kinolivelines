...
# MONTHLY TRAILING STOP, added 2026-08-06 at the user's request.
# Once the calendar month has given back this much from its own best point, open
# no NEW cycles until the month rolls over. Open positions are still managed and
# a hedge is still allowed - freezing mid-cycle would leave a stopless trade
# unattended, which is a different and worse strategy.
#
# SIZING - I got this wrong first and it is worth writing down.
# My first attempt scaled the tested $10 by account size: $10 x (30.95/1000) =
# $0.31. That is wrong. The trail is measured in DOLLARS OF TRADING P&L, and the
# lot size does not scale with the account - it is 0.01 either way, the broker
# minimum. One open position swings $1-3 whatever the balance is. A $0.31 trail
# therefore sits INSIDE the noise of a single trade and would halt the month on
# the first ordinary wobble.
#
# The tested $10 was measured at 0.01 lots. The live bot uses 0.01 lots. So the
# correct value is $10, unscaled.
#
# HONEST CONSEQUENCE at this account size: $10 is a third of a $30.95 account,
# so the trail will almost never fire before FLOOR_USD does. On a $30 account
# this rule is close to inert, and the floor is the real brake. It becomes
# meaningful once the balance is a few hundred dollars. Raise nothing to make it
# "work" - a trail that fires on single-trade noise is worse than no trail.
#
# WHAT IT DOES, honestly. On M5 it takes the account from $763 to $1,025 and
# lifts positive months from 3/10 to 5/10. On M15, $485 to $1,008 and 8/29 to
# 12/29. It turns a losing strategy into a break-even one.
#
# WHAT IT IS NOT. It is not skill at picking when to stop. A plain "do 8 trades
# a month then stop" scored $1,040 - better - and a single trade a month scored
# $1,016, against $655 for trading normally. The rule works because it cuts
# trading by about 92%, and on a negative-edge strategy anything that cuts
# trading helps. Kept because the user asked and because break-even beats
# bleeding, not because the trailing logic is finding anything.
MONTH_TRAIL_USD = 10.00      # dollars of trading P&L, NOT scaled by balance
...
def main():
    ...
    say(f"MONTH TRAIL ${MONTH_TRAIL_USD:.2f} - once the month gives back this "
        f"much from its peak, no new cycles until it rolls over "
        f"(scaled from $10 on $1,000)")
    ...
    while True:
        try:
            ...
            # ---- monthly peak, tracked on equity ---------------------------
            this_month = datetime.utcnow().strftime("%Y-%m")
            if st.get("month") != this_month:
                say(f"new month {this_month} - trailing stop reset, peak {eq:.2f}")
                st["month"] = this_month
                st["month_peak"] = eq
                save_state(st)
            elif st.get("month_peak") is None or eq > st["month_peak"]:
                st["month_peak"] = eq
                save_state(st)
            given_back = (st.get("month_peak") or eq) - eq
            month_stop = given_back >= MONTH_TRAIL_USD
            ...
            if rev and rev["time"] > st.get("last_brick", 0):
                ...
                if age > FRESH_MIN:
                    ...
                    st["last_brick"] = rev["time"]; save_state(st)
                elif month_stop and not ps:
                    # only blocks NEW cycles. A hedge for an open trade is never
                    # blocked - see the note on MONTH_TRAIL_USD.
                    say(f"MONTH TRAIL: given back {given_back:.2f} of "
                        f"{MONTH_TRAIL_USD:.2f} from this month's peak "
                        f"{st['month_peak']:.2f} - no new cycles until "
                        f"{this_month} ends")
                    st["last_brick"] = rev["time"]; save_state(st)
                elif len(ps) >= MAX_POS:
                    st["last_brick"] = rev["time"]; save_state(st)
            ...
            with open(ALIVE, "w", encoding="utf-8") as f:
                json.dump({"alive_utc": datetime.utcnow().isoformat(),
                           ...
                           "floor": FLOOR_USD,
                           "below_floor": eq <= FLOOR_USD,
                           "month": st.get("month"),
                           "month_peak": round(st.get("month_peak") or 0, 2),
                           "given_back": round(given_back, 2),
                           "month_trail": round(MONTH_TRAIL_USD, 2),
                           "month_stopped": bool(month_stop)}, f)
