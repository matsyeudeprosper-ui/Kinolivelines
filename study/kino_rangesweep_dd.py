...
print(f"BANKED max drawdown: ${_mdd:.2f} | worst float: ${max_float_dd:.2f} | min equity: ${min_equity:.2f}")
import collections, datetime as _dt
mon = collections.defaultdict(float)
for t, pnl, kind in engine:
    mon[_dt.datetime.utcfromtimestamp(t).strftime('%Y-%m')] += pnl
vals = sorted(mon.values())
print(f"months: {len(vals)} | worst ${vals[0]:.2f} | best ${vals[-1]:.2f} | positive {sum(1 for v in vals if v > 0)}/{len(vals)}")
