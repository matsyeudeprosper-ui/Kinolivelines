...
LIGHT = _sys.argv[3] if len(_sys.argv) > 3 else 'green'   # green|yellow|red
print(f'H1-FADE {LIGHT.upper()}-ONLY | {NMIN}min | START {START_BAL}', flush=True)
last_flat_hour = -1
...
for i in range(N):
    ...
    if pending is not None:
        continue
    is_green = (mode != 0 and mode == mode_d1)
    is_yellow = (mode != 0 and mode != mode_d1)
    is_red = (mode == 0)
    if LIGHT == 'green' and not is_green:
        continue
    if LIGHT == 'yellow' and not is_yellow:
        continue
    if LIGHT == 'red' and not is_red:
        continue
    if cur_hour == last_entry_hour or i < 1 or i + 1 >= N:
        continue
    rhi, rlo = closed['h1'][1], closed['h1'][2]
    if rhi - rlo <= 20:
        continue                      # dead-flat hour
    px = c[i]
    if l[i] <= rlo and c[i] > o[i]:
        pending = 1                   # touched last H1's LOW + green M1 reclaim
        last_entry_hour = cur_hour
    elif h[i] >= rhi and c[i] < o[i]:
        pending = -1                  # touched last H1's HIGH + red M1 rejection
        last_entry_hour = cur_hour
