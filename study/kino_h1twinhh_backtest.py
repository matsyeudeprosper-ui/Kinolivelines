...
print(f'H1-TWIN+HH: 2 same-color H1s making higher-high (mirror lower-low) | START {START_BAL}', flush=True)
h1_prev1 = None
h1_prev2 = None
last_flat_hour = -1
...
for i in range(N):
    ...
    if h1_prev1 is None or h1_prev2 is None:
        continue
    if mode == 1 and not (h1_prev1[3] > h1_prev1[0] and h1_prev2[3] > h1_prev2[0] and h1_prev1[1] > h1_prev2[1]):
        continue                      # need 2 green H1s making a HIGHER HIGH for buys
    if mode == -1 and not (h1_prev1[3] < h1_prev1[0] and h1_prev2[3] < h1_prev2[0] and h1_prev1[2] < h1_prev2[2]):
        continue                      # need 2 red H1s making a LOWER LOW for sells
