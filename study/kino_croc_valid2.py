...
import sys as _sys0
SPREAD = 10.0 * (float(_sys0.argv[6]) if len(_sys0.argv) > 6 else 1.0)
LOTS = 0.02
TP_PTS = 3.0 / LOTS          # 150
BE_PTS = 0.10 / LOTS         # 5
CLEAN_USD = 1.00
...
NMIN = int(_sys.argv[2]) if len(_sys.argv) > 2 else 30
print(f'VALIDATION 2: frozen pure Croc, full report | spread={SPREAD}pts | {NMIN}min | START {START_BAL}', flush=True)
entry_log = []
NRANGE = int(_sys.argv[3]) if len(_sys.argv) > 3 else 12
TRENDF = (_sys.argv[4] if len(_sys.argv) > 4 else 'on') == 'on'   # 3-same-color filter
D1F = (_sys.argv[5] if len(_sys.argv) > 5 else 'on') == 'on'      # inside-yesterday filter
h1_hist = []          # last NRANGE closed H1 candles
d1_prev = None        # last closed D1 candle
swept_lo = False      # sweep flags, reset each hour
swept_hi = False
last_flat_hour = -1
...
for i in range(N):
    for p in positions[:]:
        if i - p['idx'] >= NMIN:
            ...
            balance += pnl
            trades.append((tm[i], pnl, 'time_stop', p['dir'], i - p['idx'], p.get('mfe', 0.0), p.get('mae', 0.0)))
            positions.remove(p)
    if not positions:
        groups.clear()
        escape_active = False
    ...
    m15c = upd('m15', 900, i)
    m5c = upd('m5', 300, i)
    if m5c is not None: closed['m5'] = m5c
    d1c = upd('d1', 86400, i)
    if d1c is not None:
        d1_prev = d1c
        op1, _, _, cl1 = d1c
        ...
        if escape_active and mode != 0 and i > esc_start + 60:
            ...
            if h1dir == mode:
                for p in [x for x in positions if x['dir'] != mode]:
                    ...
                    trades.append((tm[i], pnl, 'escape_cut', p['dir'], i - p['idx'], p.get('mfe', 0.0), p.get('mae', 0.0)))
                    positions.remove(p)
                escape_active = False
                reassign()
    ...
    if pending is not None:
        ...
        entry_log.append((i, d, ep))
        positions.append({'dir': d, 'entry': ep, 'vol': 0.01, 'mfe': 0.0, 'mae': 0.0,
                          'hour': tm[i] // 3600, 'tp': ep + d * TP_PTS / 2, 'idx': i})
        positions.append({'dir': d, 'entry': ep, 'vol': 0.01, 'mfe': 0.0, 'mae': 0.0,
    ...
    for p in positions[:]:
        ...
        if hit:
            ...
            trades.append((tm[i], pnl, kind, p['dir'], i - p['idx'], p.get('mfe', 0.0), p.get('mae', 0.0)))
            positions.remove(p)
            closed_any = True
    # --- hour-team cleanup ---
    cur_hour = tm[i] // 3600
    ...
    for hid in groups_iter:
        ...
        if total >= need:
            for p in mem:
                pnl = (c[i] - p['entry']) * p['vol'] * p['dir']
                balance += pnl
                trades.append((tm[i], pnl, 'hour_clean', p['dir'], i - p['idx'], p.get('mfe', 0.0), p.get('mae', 0.0)))
                positions.remove(p)
            del groups[hid]
            closed_any = True
    if closed_any:
    ...
    for p in positions:
        exc = (c[i] - p['entry']) * p['dir']
        if exc > p.get('mfe', 0.0): p['mfe'] = exc
        if exc < p.get('mae', 0.0): p['mae'] = exc
    ...
    if cur_hour != rcp_hour:
        ...
        swept_lo = False
        swept_hi = False
    if len(h1_hist) < NRANGE or d1_prev is None:
        continue
    if pending is not None or escape_active:
        continue
    if cur_hour == last_entry_hour or i < 1 or i + 1 >= N:
        continue
    rhi = max(cd[1] for cd in h1_hist)
    rlo = min(cd[2] for cd in h1_hist)
    if rhi - rlo <= 50:
        continue                      # dead range
    # ranging-market filters:
    last3 = h1_hist[-3:]
    if TRENDF and (all(cd[3] > cd[0] for cd in last3) or all(cd[3] < cd[0] for cd in last3)):
        continue                      # 3 same-color H1s = clean trend, stand aside
    if D1F and not (d1_prev[2] <= c[i] <= d1_prev[1]):
        continue                      # price outside yesterday's range = expansion
    # sweep detection (flags persist within the hour)
    if l[i] < rlo:
        swept_lo = True
    if h[i] > rhi:
        swept_hi = True
    # reclaim = entry
    if swept_lo and c[i] > rlo and c[i] > o[i]:
        pending = 1
        last_entry_hour = cur_hour
        swept_lo = False
    elif swept_hi and c[i] < rhi and c[i] < o[i]:
        pending = -1
        last_entry_hour = cur_hour
        swept_hi = False
...
for p in positions:
    ...
    balance += pnl
    trades.append((tm[-1], pnl, 'open_end', p['dir'], (N - 1) - p['idx'], p.get('mfe', 0.0), p.get('mae', 0.0)))

engine = [t for t in trades if t[2] in ('tp', 'be', 'hour_clean', 'open_end', 'hour_flat', 'time_stop')]
tstops = [t for t in trades if t[2] == 'time_stop']
twins = [t for t in trades if t[2] in ('tp', 'be') and t[1] > 0]
print(f"TP wins: {len(twins)} (+${sum(t[1] for t in twins):,.2f}) | time-stops: {len(tstops)} (${sum(t[1] for t in tstops):,.2f})")
...
# ================= FULL TRADE-LEVEL REPORT =================
import datetime as dt, collections
det = [t for t in engine if len(t) >= 7]
pnls = np.array([t[1] for t in det])
wins = pnls[pnls > 0]; losses = pnls[pnls <= 0]
print(f"\nLEG-LEVEL: n={len(pnls)}  expectancy ${pnls.mean():+.4f}/leg  "
      f"PF {wins.sum() / max(1e-9, -losses.sum()):.3f}  win rate {(pnls > 0).mean() * 100:.1f}%")
print(f"avg winner ${wins.mean():+.4f}  avg loser ${losses.mean():+.4f}  median ${np.median(pnls):+.4f}")
hold = np.array([t[4] for t in det], dtype=float)
print(f"hold: mean {hold.mean():.1f}min  median {np.median(hold):.1f}min  "
      f"done within 5/10/15/30min: {(hold<=5).mean()*100:.0f}%/{(hold<=10).mean()*100:.0f}%/"
      f"{(hold<=15).mean()*100:.0f}%/{(hold<=30).mean()*100:.0f}%")
mfe = np.array([t[5] for t in det]) * 0.01; mae = np.array([t[6] for t in det]) * 0.01
print(f"MFE mean ${mfe.mean():+.4f} median ${np.median(mfe):+.4f} | "
      f"MAE mean ${mae.mean():+.4f} median ${np.median(mae):+.4f}")
for dname, dv in (("BUY", 1), ("SELL", -1)):
    g = [t[1] for t in det if t[3] == dv]
    if g:
        ga = np.array(g)
        print(f"{dname}: n={len(ga)}  total ${ga.sum():+.2f}  per-leg ${ga.mean():+.4f}  win {(ga>0).mean()*100:.1f}%")
yearly = collections.defaultdict(float)
for t in det: yearly[dt.datetime.utcfromtimestamp(int(t[0])).year] += t[1]
print("yearly: " + "  ".join(f"{y}: ${v:+.2f}" for y, v in sorted(yearly.items())))
mon = collections.defaultdict(float)
for t in det: mon[dt.datetime.utcfromtimestamp(int(t[0])).strftime("%Y-%m")] += t[1]
mkeys = sorted(mon); mv = np.array([mon[k] for k in mkeys])
for w in (3, 6, 12):
    roll = np.array([mv[j:j + w].mean() for j in range(len(mv) - w + 1)])
    print(f"rolling {w:2d}-mo mean $/mo: min ${roll.min():+.2f}  max ${roll.max():+.2f}  "
          f"last ${roll[-1]:+.2f}  negative windows {(roll<0).mean()*100:.0f}%")
rng = np.random.default_rng(42)
bs = np.array([rng.choice(pnls, size=len(pnls), replace=True).mean() for _ in range(10000)])
print(f"bootstrap 95% CI expectancy/leg: ${np.percentile(bs,2.5):+.4f} .. ${np.percentile(bs,97.5):+.4f}")
# ================= EXIT-VARIANT REPLAY (same frozen entries) =================
def era_of(t):
    y = dt.datetime.utcfromtimestamp(int(t)).year
    return 0 if y < 2022 else 1 if y < 2024 else 2
def report(name, per_entry_pnl):
    a = np.array(per_entry_pnl)
    eras = [0.0, 0.0, 0.0]
    for (i, d, ep), v in zip(entry_log, a): eras[era_of(tm[i])] += v
    print(f"{name}: n={len(a)}  total ${a.sum():+.2f}  per-entry {a.mean()*100:+.1f}c  "
          f"eras ${eras[0]:+.0f}/${eras[1]:+.0f}/${eras[2]:+.0f}")
print("\nEXIT REPLAY of the same frozen entries (0.02 total/entry):")
for name, tp_p, sl_p in (("B: TP+150/SL-150", 150.0, 150.0), ("C: TP+150/SL-300", 150.0, 300.0)):
    out = []
    for i0, d, ep in entry_log:
        pnl = None
        for j in range(i0, min(i0 + 1440, N)):
            if (l[j] <= ep - d * sl_p) if d == 1 else (h[j] >= ep + d * sl_p):
                pnl = -sl_p * 0.02; break
            if (h[j] >= ep + d * tp_p) if d == 1 else (l[j] <= ep + d * tp_p):
                pnl = tp_p * 0.02; break
        if pnl is None:
            j = min(i0 + 1440, N - 1); pnl = (c[j] - ep) * d * 0.02
        out.append(pnl)
    report(name, out)
for name, k in (("D: exit 5min", 5), ("E: exit 15min", 15), ("F: exit 30min", 30)):
    out = []
    for i0, d, ep in entry_log:
        j = min(i0 + k, N - 1)
        out.append((o[j] - ep) * d * 0.02)
    report(name, out)
