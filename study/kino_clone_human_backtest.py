...
FEAR = len(_sys.argv) > 2 and _sys.argv[2] == 'fear'
print(f'START_BAL {START_BAL}  HUMAN SIM (sleep 00-09, max 6/day' + (', fear-pause 24h' if FEAR else '') + ')', flush=True)
pause_until = 0
day_key = -1
day_entries = 0
...
for i in range(N):
    ...
    hod = (tm[i] // 3600) % 24
    if hod < 9:
        continue                      # human asleep 00:00-09:00
    dk = tm[i] // 86400
    if dk != day_key:
        day_key = dk
        day_entries = 0
    if day_entries >= 6:
        continue                      # human done for the day
    if FEAR and tm[i] < pause_until:
        continue                      # scared after a loss - resting 24h
