...
H1TP_MODE = _sys.argv[3] if len(_sys.argv) > 3 else 'both'   # 'both' or 'legb'
print(f'DIP+SPLIT LOCKED ({H1TP_MODE}): split TPs locked, no recovery overwrite | {NMIN}min | START {START_BAL}', flush=True)
pending_target = None
last_flat_hour = -1
...
def reassign():
    ...
    if escape_active:
        ...
        return
    for d in (1, -1):
        grp = [p for p in positions if p['dir'] == d and not p.get('lock')]
        if not grp:
            continue
...
for i in range(N):
    ...
    if pending is not None:
        ...
        tgt = pending_target
        pending_target = None
        room = (tgt - ep) * d if tgt is not None else 0.0
        if H1TP_MODE == 'fixed':
            positions.append({'dir': d, 'entry': ep, 'vol': 0.01, 'lock': True,
                              'hour': tm[i] // 3600, 'tp': ep + d * TP_PTS / 2, 'idx': i})
            positions.append({'dir': d, 'entry': ep, 'vol': 0.01, 'lock': True,
                              'hour': tm[i] // 3600, 'tp': ep + d * TP_PTS, 'idx': i})
        elif room < 10.0:
            pass                      # no room to the H1 extreme -> skip the trade
        elif H1TP_MODE == 'both':
            positions.append({'dir': d, 'entry': ep, 'vol': 0.01, 'lock': True,
                              'hour': tm[i] // 3600, 'tp': tgt, 'idx': i})
            positions.append({'dir': d, 'entry': ep, 'vol': 0.01, 'lock': True,
                              'hour': tm[i] // 3600, 'tp': tgt, 'idx': i})
        else:                         # legb: A keeps +75, B aims at the H1 extreme
            positions.append({'dir': d, 'entry': ep, 'vol': 0.01, 'lock': True,
                              'hour': tm[i] // 3600, 'tp': ep + d * TP_PTS / 2, 'idx': i})
            positions.append({'dir': d, 'entry': ep, 'vol': 0.01, 'lock': True,
                              'hour': tm[i] // 3600, 'tp': tgt, 'idx': i})
