...
GRID = float(_sys.argv[3]) if len(_sys.argv) > 3 else 500.0
print(f'ROUNDY: round-number wick+reclaim, grid {GRID} | cost {RT}pts | {NMIN}min | START {START_BAL}', flush=True)
...
for i in range(N):
    ...
    if l[i] < Lb and o[i] > Lb and c[i] > Lb and c[i] > o[i]:
        pending = 1                   # wicked through round level below, reclaimed
        last_entry_hour = cur_hour
    elif h[i] > Ub and o[i] < Ub and c[i] < Ub and c[i] < o[i]:
        pending = -1                  # wicked through round level above, rejected
        last_entry_hour = cur_hour
