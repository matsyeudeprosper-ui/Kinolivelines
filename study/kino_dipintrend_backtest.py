...
print(f'DIP-IN-TREND: H1 green + M15 red + M5 red (mirror for sells), then recipe | START {START_BAL}', flush=True)
last_flat_hour = -1
...
for i in range(N):
    ...
    if mode == 1 and not (_grn(closed['h1']) and _red(closed['m15']) and _red(closed['m5'])):
        continue                      # buys: H1 green, M15+M5 red (the dip)
    if mode == -1 and not (_red(closed['h1']) and _grn(closed['m15']) and _grn(closed['m5'])):
        continue                      # sells: H1 red, M15+M5 green (the bounce)
