...
print("building M15 bars and EMA21...")
...
print("computing M1 ATR14 for body-size check...")
        ...
        if in_pos:
            ...
            if htp or hsl:
                ...
                if not np.isnan(m1_atr[pos_sig_bar]) and m1_atr[pos_sig_bar] > 0:
                    weak_candle = bool(body < m1_atr[pos_sig_bar])

                golden = (trend_lane is True) and m1_fresh
                triple = golden and (weak_candle is True)

                trades.append((pos_et, golden, triple, usd, hsl))
                in_pos=False
    return trades, bal
...
w = sum(1 for t in trades if not t[4] and t[1]); l = sum(1 for t in trades if t[4] and t[1])
n_golden = sum(1 for t in trades if t[1]); usd_golden = sum(t[3] for t in trades if t[1])
print(f"\nGolden Lane alone (already deployed): n={n_golden} losses={l} net=${usd_golden:+,.2f}")

n_triple = sum(1 for t in trades if t[2]); l_triple = sum(1 for t in trades if t[2] and t[4])
usd_triple = sum(t[3] for t in trades if t[2])
print(f"Golden Lane + weak-candle (triple combo): n={n_triple} losses={l_triple} net=${usd_triple:+,.2f}")

n_dropped = n_golden - n_triple
usd_dropped_from_golden = usd_golden - usd_triple
l_dropped = l - l_triple
print(f"\nWhat the 3rd filter would drop FROM Golden Lane: n={n_dropped} losses_removed={l_dropped} usd_removed=${usd_dropped_from_golden:+,.2f}")
...
print("\n=== Triple combo, era by era ===")
tot=0.0
for label,d0,d1 in eras:
    grp = [t for t in trades if d0<=t[0]<d1 and t[2]]
    gn = sum(t[3] for t in grp); gl = sum(1 for t in grp if t[4])
    tot += gn
