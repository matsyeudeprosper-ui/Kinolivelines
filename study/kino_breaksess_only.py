...
SESS = _sys.argv[3] if len(_sys.argv) > 3 else 'asia'   # asia|london|ny
SESS_LO, SESS_HI = {'asia': (0, 8), 'london': (8, 16), 'ny': (16, 24)}[SESS]
print(f'BREAKOUT {SESS.upper()}-ONLY: entries only {SESS_LO:02d}-{SESS_HI:02d} UTC | {NMIN}min | START {START_BAL}', flush=True)
last_flat_hour = -1
...
for i in range(N):
    for p in positions[:]:
        if i - p['idx'] >= NMIN:
            ...
            balance += pnl
            trades.append((tm[i], pnl, 'time_stop', tm[p['idx']]))
            positions.remove(p)
    if not positions:
        groups.clear()
        escape_active = False
        ...
        if escape_active and mode != 0 and i > esc_start + 60:
            ...
            if h1dir == mode:
                for p in [x for x in positions if x['dir'] != mode]:
                    ...
                    trades.append((tm[i], pnl, 'escape_cut', tm[p['idx']]))
                    positions.remove(p)
                escape_active = False
                reassign()
    ...
    for p in positions[:]:
        ...
        if hit:
            ...
            trades.append((tm[i], pnl, kind, tm[p['idx']]))
            positions.remove(p)
            closed_any = True
    # --- hour-team cleanup ---
    cur_hour = tm[i] // 3600
    ...
    for hid in groups_iter:
        ...
        if total >= need:
            for p in mem:
                pnl = (c[i] - p['entry']) * p['vol'] * p['dir']
                balance += pnl
                trades.append((tm[i], pnl, 'hour_clean', tm[p['idx']]))
                positions.remove(p)
            del groups[hid]
            closed_any = True
    if closed_any:
    ...
    if cur_hour != rcp_hour:
        ...
        rcp_setup = None
        rcp_pulled = False
    if 'h1' in closed and closed.get('_h1_id') != id(closed['h1']):
        if closed.get('_h1_cur') is not closed['h1']:
            closed['_h1_prev'] = closed.get('_h1_cur')
            closed['_h1_cur'] = closed['h1']
        closed['_h1_id'] = id(closed['h1'])
    if closed.get('_h1_prev') is None or 'h1' not in closed:
        continue
    def _grn(cd): return cd[3] > cd[0]
    def _red(cd): return cd[3] < cd[0]
    c2, c1 = closed['h1'], closed['_h1_prev']   # c2 = last closed, c1 = one before
    if mode == 1 and not (_grn(c1) and _grn(c2) and c2[3] > c1[1]):
        continue                      # twin greens, 2nd CLOSES above 1st's high
    if mode == -1 and not (_red(c1) and _red(c2) and c2[3] < c1[2]):
        continue                      # twin reds, 2nd CLOSES below 1st's low
    ...
    if cur_hour == last_entry_hour or i < 1 or i + 1 >= N:
        continue
    if not (SESS_LO <= (tm[i] // 3600) % 24 < SESS_HI):
        continue
    d = mode
    if d == 1:
        g_prev = c[i-1] > o[i-1]
...
for p in positions:
    ...
    balance += pnl
    trades.append((tm[-1], pnl, 'open_end', tm[p['idx']]))
...
def sess(ts):
    ...
    return 'Asia(00-08)' if h < 8 else 'London(08-16)' if h < 16 else 'NewYork(16-24)'
def era(ts):
    y = _dt.datetime.utcfromtimestamp(ts).year
    return 0 if y < 2022 else 1 if y < 2024 else 2
tab = {}
for t in engine:
    if len(t) < 4: continue
    s = sess(t[3]); e = era(t[3])
    d = tab.setdefault(s, {'n': 0, 'pnl': 0.0, 'eras': [0.0, 0.0, 0.0]})
    d['n'] += 1; d['pnl'] += t[1]; d['eras'][e] += t[1]
print()
for s in ('Asia(00-08)', 'London(08-16)', 'NewYork(16-24)'):
    d = tab.get(s, {'n': 0, 'pnl': 0.0, 'eras': [0, 0, 0]})
    per = d['pnl'] / d['n'] * 100 if d['n'] else 0.0
    print(f"{s:15s} closes={d['n']:5d}  pnl=${d['pnl']:+8.2f}  per-close={per:+.1f}c  eras: "
          + " ".join(f"${x:+.0f}" for x in d['eras']))
