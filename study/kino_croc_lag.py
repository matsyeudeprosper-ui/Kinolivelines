...
for i in range(N):
    for p in positions[:]:
        if i - p['idx'] >= NMIN:
            ...
            balance += pnl
            trades.append((tm[i], pnl, 'time_stop', p.get('lag')))
            positions.remove(p)
    if not positions:
        groups.clear()
        escape_active = False
        ...
        if escape_active and mode != 0 and i > esc_start + 60:
            ...
            if h1dir == mode:
                for p in [x for x in positions if x['dir'] != mode]:
                    ...
                    trades.append((tm[i], pnl, 'escape_cut', p.get('lag')))
                    positions.remove(p)
                escape_active = False
                reassign()
    ...
    for p in positions[:]:
        ...
        if hit:
            ...
            trades.append((tm[i], pnl, kind, p.get('lag')))
            positions.remove(p)
            closed_any = True
    # --- hour-team cleanup ---
    cur_hour = tm[i] // 3600
    ...
    for hid in groups_iter:
        ...
        if total >= need:
            for p in mem:
                pnl = (c[i] - p['entry']) * p['vol'] * p['dir']
                balance += pnl
                trades.append((tm[i], pnl, 'hour_clean', p.get('lag')))
                positions.remove(p)
            del groups[hid]
            closed_any = True
    if closed_any:
