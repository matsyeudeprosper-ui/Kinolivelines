...
print(f'TWIN-BREAKOUT: 2 same-color H1s, 2nd closes beyond 1st extreme | START {START_BAL}', flush=True)
h1_prev1 = None
h1_prev2 = None
last_flat_hour = -1
...
for i in range(N):
    ...
    if mode == 1 and not (h1_prev1[3] > h1_prev1[0] and h1_prev2[3] > h1_prev2[0] and h1_prev1[3] > h1_prev2[1]):
        continue                      # 2 greens AND 2nd CLOSES above 1st's HIGH
    if mode == -1 and not (h1_prev1[3] < h1_prev1[0] and h1_prev2[3] < h1_prev2[0] and h1_prev1[3] < h1_prev2[2]):
        continue                      # 2 reds AND 2nd CLOSES below 1st's LOW
