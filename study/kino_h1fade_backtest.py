...
print(f'H1-FADE on YELLOW/RED: buy last-H1 low / sell last-H1 high, non-green light only, all hours | {NMIN}min | START {START_BAL}', flush=True)
last_flat_hour = -1
...
for i in range(N):
    ...
    if pending is not None:
        continue
    if mode != 0 and mode == mode_d1:
        continue                      # GREEN light -> stand aside (yellow/red only)
