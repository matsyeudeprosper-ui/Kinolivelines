...
print(f'BE-AT-HALF test: $3 game, SL->entry at +75pts, time-stop {NMIN}min | START {START_BAL}', flush=True)
last_flat_hour = -1
...
for i in range(N):
    for p in positions[:]:
        if not p.get('armed'):
            fav = (h[i] - p['entry']) if p['dir'] == 1 else (p['entry'] - l[i])
            if fav >= TP_PTS / 2:
                p['armed'] = True
                p['besl'] = p['entry'] + p['dir'] * 5.0
        if p.get('armed'):
            hit_be = (l[i] <= p['besl']) if p['dir'] == 1 else (h[i] >= p['besl'])
            if hit_be:
                pnl = (p['besl'] - p['entry']) * p['vol'] * p['dir']
                balance += pnl
                trades.append((tm[i], pnl, 'be_out'))
                positions.remove(p)
                continue
        if i - p['idx'] >= NMIN:
...
engine = engine + [t for t in trades if t[2] == 'be_out']
tstops = [t for t in trades if t[2] == 'time_stop']
twins = [t for t in trades if t[2] in ('tp', 'be') and t[1] > 0]
beouts = [t for t in trades if t[2] == 'be_out']
print(f"TP wins: {len(twins)} (+${sum(t[1] for t in twins):,.2f}) | BE-outs: {len(beouts)} (+${sum(t[1] for t in beouts):,.2f}) | time-stops: {len(tstops)} (${sum(t[1] for t in tstops):,.2f})")
