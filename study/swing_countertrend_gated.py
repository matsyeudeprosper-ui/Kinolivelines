...
print("deriving genuine swing highs/lows and a causal trend state...")
trend_events = []
...
def run_full(o,h,l,c,tm,N,sigs,trig_f,cap_f,mode):
    """mode: 'none'=no gate, 'with'=only trade WITH the established trend
    (already tested, failed), 'against'=only trade AGAINST the established
    trend (the flipped hypothesis - a BUY only when the broader trend is
    DOWN, a SELL only when the broader trend is UP; ranging/unknown blocked
    either way, same as the 'with' test, for a clean apples-to-apples
    comparison)."""
...
t0, n0 = run_full(o_f,h_f,l_f,c_f,tm_f,N,sigs,0.30,1.00,'none')
calendar_stats(t0, "RATCHET ONLY, no trend gate (current live)")

print("running RATCHET + COUNTER-TREND GATE (real gating - the flipped hypothesis)...")
t1, n1 = run_full(o_f,h_f,l_f,c_f,tm_f,N,sigs,0.30,1.00,'against')
gated = calendar_stats(t1, "RATCHET + AGAINST-THE-TREND GATE (REAL GATE)")
