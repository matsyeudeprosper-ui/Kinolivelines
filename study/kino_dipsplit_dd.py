...
print(f'DIP+SPLIT: H1grn/M15red/M5red gate + split TP + {NMIN}min | START {START_BAL}', flush=True)
last_flat_hour = -1
...
last_entry_hour = -1
min_equity = 1e9
_pk = 0.0; _mdd = 0.0; _eqpk = 0.0; _eqmdd = 0.0
max_float_dd = 0.0
deaths = 0
death_dates = []
max_conc = 0
...
for i in range(N):
    ...
    if positions:
        ...
        # banked-curve drawdown (closed pnl running peak-to-valley)
        if balance > _pk: _pk = balance
        if _pk - balance > _mdd: _mdd = _pk - balance
        _eqpk = max(_eqpk, eq)
        _eqmdd = max(_eqmdd, _eqpk - eq)
...
print(f"BANKED max drawdown: ${_mdd:.2f} | EQUITY max drawdown (incl float): ${_eqmdd:.2f} | worst float: ${max_float_dd:.2f} | min equity: ${min_equity:.2f}")
import collections
...
print(f"months: {len(vals)} | worst month ${vals[0]:.2f} | best month ${vals[-1]:.2f} | positive months {sum(1 for v in vals if v > 0)}/{len(vals)}")
