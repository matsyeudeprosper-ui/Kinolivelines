"""Per-signal EV by regime for ANY 2:1 (or other) shape. Args: TP_pct SL_pct
e.g. 1 0.5 - every signal simulated independently, bucketed by 24h-range
regime and era. The clean measurement (no cap=1 lottery)."""
import sys, json
import numpy as np
...
LOTS = 0.05
ATR_WIN = 1440
TP_PCT = float(sys.argv[1])/100.0
SL_PCT = float(sys.argv[2])/100.0
...
print(f"shape: TP {100*TP_PCT:g}% / SL {100*SL_PCT:g}%", flush=True)
...
print("simulating every signal independently...", flush=True)
CHUNK = 20000
results = []
durs = []
...
print(f"resolved: {len(results)}   median trade duration: {int(np.median(durs))} min   mean: {int(np.mean(durs))} min", flush=True)
...
be = 100.0*TP_PCT/(TP_PCT+SL_PCT)
print(f"\n(breakeven loss rate for this shape: {be:.2f}%)")
print(f"{'regime':<10} {'n':>6} {'loss%':>7} {'avg$/sig':>9} {'total$':>9}")
for lo, hi in BUCKETS:
    ...
    print(f"{f'{lo}-{hi}%':<10} {n:>6} {100*losses/n:>6.2f}% {tot/n:>9.3f} {tot:>9,.0f}")
print("\nper era:")
