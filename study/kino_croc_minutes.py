...
print(f'RANGE SWEEP V1: 12-H1 range, M1 sweep+reclaim, ranging-only filters | {NMIN}min | START {START_BAL}', flush=True)
...
sweep_t = None        # when the current hour's first sweep happened
pending_lag = None
...
for i in range(N):
    for p in positions[:]:
        if i - p['idx'] >= NMIN:
            ...
            balance += pnl
            trades.append((tm[i], pnl, 'time_stop', p.get('lag'), (tm[p['idx']] // 60) % 60))
            positions.remove(p)
    if not positions:
        groups.clear()
        escape_active = False
        ...
        if escape_active and mode != 0 and i > esc_start + 60:
            ...
            if h1dir == mode:
                for p in [x for x in positions if x['dir'] != mode]:
                    ...
                    trades.append((tm[i], pnl, 'escape_cut', p.get('lag'), (tm[p['idx']] // 60) % 60))
                    positions.remove(p)
                escape_active = False
                reassign()
    ...
    if pending is not None:
        ...
        positions.append({'dir': d, 'entry': ep, 'vol': 0.01, 'lag': pending_lag,
    ...
    for p in positions[:]:
        ...
        if hit:
            ...
            trades.append((tm[i], pnl, kind, p.get('lag'), (tm[p['idx']] // 60) % 60))
            positions.remove(p)
            closed_any = True
    # --- hour-team cleanup ---
    cur_hour = tm[i] // 3600
    ...
    for hid in groups_iter:
        ...
        if total >= need:
            for p in mem:
                pnl = (c[i] - p['entry']) * p['vol'] * p['dir']
                balance += pnl
                trades.append((tm[i], pnl, 'hour_clean', p.get('lag'), (tm[p['idx']] // 60) % 60))
                positions.remove(p)
            del groups[hid]
            closed_any = True
    if closed_any:
    ...
    if l[i] < rlo:
        if not swept_lo: sweep_t = tm[i]
        swept_lo = True
    if h[i] > rhi:
        if not swept_hi: sweep_t = tm[i]
        swept_hi = True
    # reclaim = entry
    if swept_lo and c[i] > rlo and c[i] > o[i]:
        pending = 1
        pending_lag = (tm[i] - sweep_t) / 60.0 if sweep_t else 0.0
...
buckets = [(0, 3, '0-3min '), (3, 10, '3-10min'), (10, 30, '10-30min'), (30, 999, '30min+ ')]
print()
for lo_b, hi_b, name in buckets:
    grp = [t for t in engine if len(t) > 3 and t[3] is not None and lo_b <= t[3] < hi_b]
    n = len(grp); s = sum(t[1] for t in grp)
    per = s / n * 100 if n else 0.0
    print(f"lag {name}: closes={n:5d}  pnl=${s:+8.2f}  per-close={per:+.1f}c")

print()
import datetime as _dt
for lo_m, hi_m, name in ((0, 10, ':00-:09'), (10, 20, ':10-:19'), (20, 30, ':20-:29'),
                          (30, 45, ':30-:44'), (45, 60, ':45-:59')):
    grp = [t for t in engine if len(t) > 4 and lo_m <= t[4] < hi_m]
    n = len(grp); s = sum(t[1] for t in grp)
    per = s / n * 100 if n else 0.0
    # era split for the bucket
    eras = [0.0, 0.0, 0.0]
    for t in grp:
        y = _dt.datetime.utcfromtimestamp(t[0]).year
        eras[0 if y < 2022 else 1 if y < 2024 else 2] += t[1]
    print(f"entry {name}: closes={n:5d}  pnl=${s:+8.2f}  per-close={per:+.1f}c  eras: "
          + " ".join(f"${x:+.0f}" for x in eras))
