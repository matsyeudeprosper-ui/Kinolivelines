...
G4H = float(_sys.argv[2]) if len(_sys.argv) > 2 else 99.0   # 4h-range calm gate %
CAP = _sys.argv[3] if len(_sys.argv) > 3 else 'none'         # none|onewin|day9
print(f'SCALPER TEST: hour-flat + 4h-calm gate {G4H}% + cap {CAP} | START {START_BAL}', flush=True)
...
for i in range(N):
    ...
    if cur_hour != rcp_hour:
        ...
        rcp_pulled = False
    if range4h >= G4H:
        continue                      # scalper calm gate (4h window)
    if CAP == 'onewin' and (tm[i] // 3600) == hour_won:
        continue                      # already banked a win this hour
    if CAP == 'day9' and (day_pnl >= 9.0 or day_pnl <= -9.0):
        continue                      # day target/stop reached
