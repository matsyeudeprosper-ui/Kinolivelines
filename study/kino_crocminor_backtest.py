...
print(f'CROC MINOR: 6-box sweeps that do NOT reach the 12-box | cost {RT}pts | {NMIN}min | START {START_BAL}', flush=True)
...
for i in range(N):
    ...
    if cur_hour == last_entry_hour or i < 1 or i + 1 >= N:
        continue
    rhi = max(cd[1] for cd in h1_hist[-6:])
    rlo = min(cd[2] for cd in h1_hist[-6:])
    rhi12 = max(cd[1] for cd in h1_hist)
    rlo12 = min(cd[2] for cd in h1_hist)
    if rhi - rlo <= 50:
        continue                      # dead range
    ...
    if l[i] < rlo and l[i] > rlo12:
        swept_lo = True               # swept the 6-box floor but NOT the 12-box
    if h[i] > rhi and h[i] < rhi12:
        swept_hi = True
    # reclaim = entry
