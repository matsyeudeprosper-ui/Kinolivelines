...
# --- get the established segment date boundaries (same H1 6-way split used all session) ---
...
for i,(d0,d1) in enumerate(seg_dates,1):
    print(f"segment {i}: {d0} to {d1}")

# --- M1 data, live-matching mechanism ---
...
tm_f = np.array(times)
print(f"\nM1 full range: {datetime.utcfromtimestamp(tm_f[0])} -> {datetime.utcfromtimestamp(tm_f[-1])}\n")
...
for i,(d0,d1) in enumerate(seg_dates,1):
    ...
    if mask.sum() < 100:
        print(f"segment {i}: not enough M1 data in this window, skipping")
        continue
    ...
    zh = run_with_hedge(ot,ht,lt,ct,tmt,Nt,sigt)
    print(f"\n=== SEGMENT {i}: {d0.date()} to {d1.date()} ===")
    print(f"  BASELINE:     {zb['trades']:>4} trades  {zb['losses']} losses  net ${zb['net']:>10,.2f}")
    for L,t,usd in zb['loss_events']:
        print(f"      LOSS: {'BUY' if L else 'SELL'} {t}  ${usd}")
    print(f"  WITH HEDGE:   primary {zh['trades']:>4}tr/{zh['losses']}L + hedge {zh['hedge_trades']:>4}tr/{zh['hedge_losses']}L  "
          f"TOTAL net ${zh['net']:>10,.2f}   (diff: ${zh['net']-zb['net']:+,.2f})")
    for kind,L,t,usd in zh['loss_events']:
        print(f"      [{kind}] LOSS: {'BUY' if L else 'SELL'} {t}  ${usd}")
print("="*100)
