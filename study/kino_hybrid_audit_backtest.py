...
print(f'PURE HYBRID (no hour-clean, no hedge/escape): recipe + TP1%/SL0.5% + gate {GATE_PCT}% | START {START_BAL}', flush=True)
...
import collections
kinds = collections.Counter(t[2] for t in trades)
print('close kinds:', dict(kinds))
for k in kinds:
    s = sum(t[1] for t in trades if t[2] == k)
    print(f'  {k}: n={kinds[k]} sum=${s:,.2f}')
