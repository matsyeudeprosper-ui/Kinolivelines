...
h1_hist = []          # last 12 closed H1 candles
...
for i in range(N):
    ...
    if cur_hour != rcp_hour:
        ...
        swept_lo = False
        swept_hi = False
    if len(h1_hist) < 12 or d1_prev is None:
        continue
    ...
    last3 = h1_hist[-3:]
    if all(cd[3] > cd[0] for cd in last3) or all(cd[3] < cd[0] for cd in last3):
        continue                      # 3 same-color H1s = clean trend, stand aside
    if not (d1_prev[2] <= c[i] <= d1_prev[1]):
