"""V2 DD-brake test: uniform drawdown throttle on the animal bots' shapes.
Lot multiplier by current realized drawdown from peak:
  DD < A       -> 1.00x
  A <= DD < 2A -> 0.50x
  DD >= 2A     -> 0.25x
Sizes recover automatically as the account climbs back. Uniform rule
(applies to every trade identically) - the surviving category.

Args: profile A   e.g.  rabbit 1000   (A=0 -> no throttle, baseline)
"""
import sys, json
import numpy as np
import MetaTrader5 as mt5
from datetime import datetime, timedelta
from collections import deque, defaultdict

BRICK, REVERSAL = 50.0, 2
SPREAD_PTS = 10.0
BASE_LOTS = 0.05
ATR_WIN = 1440

PROFILES = {
    "turtle": dict(TP_PCT=0.01, SL_PCT=0.005, GATE_PCT=2.5, MAX_POS=5),
    "rabbit": dict(TP_PCT=0.04, SL_PCT=0.02,  GATE_PCT=4.0, MAX_POS=10),
}
NAME = sys.argv[1]; A = float(sys.argv[2])
P = PROFILES[NAME]

files = ['coinbase_m1_2yr_partneg1.json','coinbase_m1_2yr_part0.json','coinbase_m1_2yr_part1.json',
         'coinbase_m1_2yr_part2.json','coinbase_m1_extra_year.json','coinbase_m1_pilot.json']
rows = {}
for f in files:
    for t, lo, hi, op, cl, vol in json.load(open(f)):
        rows[int(t)] = (op, hi, lo, cl)
ok = mt5.initialize(path=r"C:\Program Files\MetaTrader 5\terminal64.exe")
mt5.symbol_select("BTCUSDm", True)
r = mt5.copy_rates_from_pos("BTCUSDm", mt5.TIMEFRAME_M1, 0, 99000)
mt5.shutdown()
for i in range(len(r)):
    t = int(r["time"][i])
    rows[t] = (float(r["open"][i]), float(r["high"][i]), float(r["low"][i]), float(r["close"][i]))
times = sorted(rows.keys())
o_f = np.array([rows[t][0] for t in times]); h_f = np.array([rows[t][1] for t in times])
l_f = np.array([rows[t][2] for t in times]); c_f = np.array([rows[t][3] for t in times])
tm_f = np.array(times)
N = len(times)

def build_bricks_signals(o,h,l,c,N):
    revs = {}
    ao = ac = float(o[0]); d = 0; pd_ = 0
    for i in range(N):
        while True:
            up = (ao if d==-1 else ac) + BRICK*(REVERSAL if d==-1 else 1)
            dn = (ao if d==1 else ac) - BRICK*(REVERSAL if d==1 else 1)
            if c[i] >= up:
                base = ao if d==-1 else ac; ao,ac,d = base, base+BRICK, 1
            elif c[i] <= dn:
                base = ao if d==1 else ac; ao,ac,d = base, base-BRICK, -1
            else: break
            if pd_ and d != pd_: revs.setdefault(i,d)
            pd_ = d
    return revs

print(f"profile={NAME}  throttle A=${A:g} (0=off)", flush=True)
print("building signals...", flush=True)
sigs = build_bricks_signals(o_f,h_f,l_f,c_f,N)

print("computing 24h range...", flush=True)
range24 = np.full(N, np.nan)
maxq = deque(); minq = deque()
for j in range(N):
    while maxq and h_f[maxq[-1]] <= h_f[j]: maxq.pop()
    maxq.append(j)
    while minq and l_f[minq[-1]] >= l_f[j]: minq.pop()
    minq.append(j)
    lo_bound = j - ATR_WIN + 1
    while maxq[0] < lo_bound: maxq.popleft()
    while minq[0] < lo_bound: minq.popleft()
    if j >= ATR_WIN:
        range24[j] = h_f[maxq[0]] - l_f[minq[0]]

def lot_mult(dd):
    if A <= 0: return 1.0
    if dd < A: return 1.0
    if dd < 2*A: return 0.5
    return 0.25

print("running...", flush=True)
bal=0.0; peak=0.0; mdd=0.0
open_pos = []; pending=None; trades = []
for j in range(N):
    if pending is not None:
        L, et, lots = pending; pending = None
        entry = o_f[j] + (SPREAD_PTS if L else 0.0)
        open_pos.append(dict(L=L, entry=entry, lots=lots,
                             tp=entry*(1+P['TP_PCT']) if L else entry*(1-P['TP_PCT']),
                             sl=entry*(1-P['SL_PCT']) if L else entry*(1+P['SL_PCT']),
                             et=et))
    if j in sigs and j+1<N and len(open_pos) < P['MAX_POS']:
        if not np.isnan(range24[j]) and 100.0*range24[j]/c_f[j] < P['GATE_PCT']:
            dd_now = peak - bal
            pending = ((sigs[j]==1), int(tm_f[j+1]), BASE_LOTS*lot_mult(dd_now))
    still = []
    for p in open_pos:
        if p['L']:
            htp = h_f[j] >= p['tp']; hsl = l_f[j] <= p['sl']
        else:
            htp = l_f[j] <= p['tp']; hsl = h_f[j] >= p['sl']
        if htp or hsl:
            usd = (-p['entry']*P['SL_PCT'] if hsl else p['entry']*P['TP_PCT'])*p['lots']
            bal += usd
            if bal > peak: peak = bal
            if peak-bal > mdd: mdd = peak-bal
            trades.append((int(tm_f[j]), usd, hsl))
        else:
            still.append(p)
    open_pos = still

n = len(trades)
losses = sum(1 for t in trades if t[2]); wins = n-losses
monthly = defaultdict(float)
for xt, usd, hsl in trades:
    dt = datetime.utcfromtimestamp(xt)
    monthly[(dt.year, dt.month)] += usd
mv = [v for k,v in sorted(monthly.items())]
yearly = defaultdict(float)
for (y,m),v in monthly.items(): yearly[y]+=v
span_days = (tm_f[-1]-tm_f[0])/86400
print(f"\nRESULT {NAME} A=${A:g}: trades={n} W={wins} L={losses}  "
      f"net=${bal:,.2f}  $/mo=${bal/(span_days/30.44):,.2f}  maxDD=${mdd:,.2f}  "
      f"months+ {sum(1 for v in mv if v>0)}/{len(mv)}", flush=True)
print("yearly: " + "  ".join(f"{y}:{v:+,.0f}" for y,v in sorted(yearly.items())))
