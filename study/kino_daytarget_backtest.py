...
DAYTGT = float(_sys.argv[3]) if len(_sys.argv) > 3 else 6.0
print(f'DAY-TARGET: stop for the day at +{DAYTGT} net, keep hunting while below | START {START_BAL}', flush=True)
day_led = {}
last_flat_hour = -1
...
for i in range(N):
    ...
    if balance - day_led['base'] >= DAYTGT:
        continue                      # day target reached: done for the day
