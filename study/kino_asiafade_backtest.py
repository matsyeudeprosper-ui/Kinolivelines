...
print(f'ASIA RANGE-FADE: buy low 15% / sell high 15% of last-closed-H4 range, 00-08 UTC only, no light | {NMIN}min | START {START_BAL}', flush=True)
last_flat_hour = -1
...
for i in range(N):
    ...
    if pending is not None or escape_active:
        continue                      # range-fade ignores the light on purpose
    if cur_hour == last_entry_hour or i < 1 or i + 1 >= N:
        continue
    if (tm[i] // 3600) % 24 >= 8:
        continue                      # Asia hours only
    rhi, rlo = closed['h4'][1], closed['h4'][2]
    rw = rhi - rlo
    if rw <= 50:
        continue                      # dead-flat range: nothing to fade
    px = c[i]
    if px <= rlo + 0.15 * rw and c[i] > o[i]:
        pending = 1                   # near range bottom + green M1 reclaim
        last_entry_hour = cur_hour
    elif px >= rhi - 0.15 * rw and c[i] < o[i]:
        pending = -1                  # near range top + red M1 rejection
        last_entry_hour = cur_hour
