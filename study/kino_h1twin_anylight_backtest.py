...
print(f'H1-TWIN on ANY light (green+yellow): 2 same-color H1s + H4 direction | START {START_BAL}', flush=True)
h1_prev1 = None
h1_prev2 = None
last_flat_hour = -1
...
for i in range(N):
    ...
    if h1_prev1 is None or h1_prev2 is None:
        continue
    if mode == 1 and not (h1_prev1[3] > h1_prev1[0] and h1_prev2[3] > h1_prev2[0]):
        continue                      # need 2 green H1s for buys
    if mode == -1 and not (h1_prev1[3] < h1_prev1[0] and h1_prev2[3] < h1_prev2[0]):
        continue                      # need 2 red H1s for sells
    if mode == 0 or pending is not None or escape_active:
        continue                      # ANY light (green or yellow), H4 sets direction
