...
print(f'RAW-EXEC: symmetric round-trip cost {RT}pts front-loaded | {NMIN}min | START {START_BAL}', flush=True)
...
_days = {}
for t in engine:
    _dk = _dt2.datetime.utcfromtimestamp(int(t[0])).strftime("%Y-%m-%d")
    _days[_dk] = _days.get(_dk, 0.0) + t[1]
_json2.dump(_days, open("croc_daily_std.json", "w"))
print("daily pnl dumped -> croc_daily.json")
