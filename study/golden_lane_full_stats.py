...
def run_full(o,h,l,c,tm,N,sigs,trig_f,cap_f,use_golden_lane):
    ...
    for j in range(N):
        ...
        if j in sigs and j+1<N and not in_pos:
            ...
            if use_golden_lane:
                ...
                golden_ok = (trend_lane is True) and m1_fresh
            else:
                golden_ok = True
            if golden_ok:
...
def calendar_stats(trades, label):
    ...
    if trades:
        durs = [(xt-et)/60.0 for et,xt,_,_,_ in trades]
        print(f"trade duration (min): median={sorted(durs)[len(durs)//2]:,.1f}  mean={sum(durs)/len(durs):,.1f}  max={max(durs):,.1f}")

print("\nrunning OLD RULE (no ratchet, no golden lane - the pre-2026-08-17 baseline)...")
t0, n0 = run_full(o_f,h_f,l_f,c_f,tm_f,N,sigs,None,None,False)
calendar_stats(t0, "OLD BASELINE (relative 40% SL only)")

print("running RATCHET ONLY (deployed 2026-08-18, no golden lane)...")
t1, n1 = run_full(o_f,h_f,l_f,c_f,tm_f,N,sigs,0.30,1.00,False)
calendar_stats(t1, "RATCHET ONLY (trigger=30%, cap=100%)")

print("running CURRENT LIVE RULE: RATCHET + GOLDEN LANE (deployed 2026-08-19)...")
t2, n2 = run_full(o_f,h_f,l_f,c_f,tm_f,N,sigs,0.30,1.00,True)
calendar_stats(t2, "CURRENT LIVE RULE: RATCHET + GOLDEN LANE")
