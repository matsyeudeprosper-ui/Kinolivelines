...
print(f'DEMO2 SIM: H1-color direction, no recovery, hour-flat, spent-hour filter={FILT} | START {START_BAL}', flush=True)
hr_hi = -1e18
hr_lo = 1e18
hr_id_track = -1
last_flat_hour = -1
...
for i in range(N):
    ...
    if FILT:
        if h1dir == 1 and _cond_hi >= _h1[1]:
            continue                  # hour already took the prev H1 high: spent
        if h1dir == -1 and _cond_lo <= _h1[2]:
            continue                  # hour already took the prev H1 low: spent
