        ...
        if in_pos:
            ...
            if htp or hsl:
                ...
                if p1 is not None and not np.isnan(m15_ema[p1]):
                    above = bool(m15_c[p1] > m15_ema[p1])
                trades.append((pos_et, above, usd, hsl))
                in_pos=False
    return trades, bal
...
print("\n=== M15 above/below EMA21, sliced by era (one continuous run, sliced post-hoc) ===")
tot_below = 0.0; tot_above = 0.0
for label, d0, d1 in eras:
    below = [t for t in trades if d0<=t[0]<d1 and t[1] is False]
    above = [t for t in trades if d0<=t[0]<d1 and t[1] is True]
    bn = sum(t[2] for t in below); bl = sum(1 for t in below if t[3])
    an = sum(t[2] for t in above); al = sum(1 for t in above if t[3])
    tot_below += bn; tot_above += an
    print(f"{label}: BELOW n={len(below):>4} loss={bl:>2} net=${bn:>+9,.2f}   ABOVE n={len(above):>4} loss={al:>2} net=${an:>+9,.2f}")
print(f"\nTOTAL below: ${tot_below:+,.2f}   TOTAL above: ${tot_above:+,.2f}")
