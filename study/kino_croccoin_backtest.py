...
import sys as _s0
RT = float(_s0.argv[6]) if len(_s0.argv) > 6 else 0.0   # full round-trip cost, pts
LOTS = 0.02
TP_PTS = 3.0 / LOTS          # 150
BE_PTS = 0.10 / LOTS         # 5
CLEAN_USD = 1.00
...
DAYTGT = float(_s0.argv[7]) if len(_s0.argv) > 7 else 1.5
print(f'CROC COIN-COLLECTOR: stop for the day at +${DAYTGT} banked | cost {RT}pts | START {START_BAL}', flush=True)
day_led = {}
...
for i in range(N):
    for p in positions[:]:
        if i - p['idx'] >= NMIN:
            ...
            balance += pnl
            trades.append((tm[i], pnl, 'time_stop'))
            positions.remove(p)
    if not positions:
        groups.clear()
        escape_active = False
    ...
    if pending is not None:
        d = pending
        ep = o[i] + d * RT
        positions.append({'dir': d, 'entry': ep, 'vol': 0.01,
    ...
    if positions:
        ...
        if balance > _pk: _pk = balance
        if _pk - balance > _mdd: _mdd = _pk - balance
    ...
    # reclaim = entry
    _day = tm[i] // 86400
    if day_led.get('d') != _day:
        day_led['d'] = _day
        day_led['base'] = balance
    if balance - day_led['base'] >= DAYTGT:
        continue                      # coin collected: done for the day
...
pnls = np.array([t[1] for t in engine])
wins = pnls[pnls > 0]; losses = pnls[pnls <= 0]
print(f"expectancy ${pnls.mean():+.4f}/leg | PF {wins.sum() / max(1e-9, -losses.sum()):.3f} | "
      f"win {(pnls > 0).mean() * 100:.1f}% | max banked DD ${_mdd:.2f}")

import collections, datetime as _dt
days = collections.defaultdict(float)
for t in engine:
    days[_dt.datetime.utcfromtimestamp(int(t[0])).strftime('%Y-%m-%d')] += t[1]
dv = sorted(days.values())
n_green = sum(1 for v in dv if v > 0)
print(f"DAYS: {len(dv)} traded | GREEN {n_green} ({n_green / len(dv) * 100:.0f}%) | "
      f"avg day ${sum(dv) / len(dv):+.3f} | worst day ${dv[0]:.2f} | best day ${dv[-1]:.2f}")
