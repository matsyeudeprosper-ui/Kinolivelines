...
TP_PCT = 0.005
SL_PCT = 0.0025
TP_PTS = 3.0 / LOTS          # (unused for hybrid positions)
BE_PTS = 0.10 / LOTS         # 5
CLEAN_USD = 1.00
CLEAN_PTS = 50.0
import sys as _sys
...
print(f'FAST-TURTLE: BRICK entry + TP0.5%/SL0.25% + 4h-calm gate {GATE_PCT}% | START {START_BAL}', flush=True)
...
for i in range(N):
    ...
    if pending is not None:
        ...
        positions.append({'dir': d, 'entry': ep, 'vol': LOTS,
                          'hour': tm[i] // 3600, 'tp': ep + d * ep * TP_PCT,
                          'sl': ep - d * ep * SL_PCT, 'idx': i})
        groups.setdefault(tm[i] // 3600, 0.0)
        pending = None
        reassign()
    ...
    for p in positions[:]:
        slp = p.get('sl')
        if slp is not None:
            hit_sl = (l[i] <= slp) if p['dir'] == 1 else (h[i] >= slp)
            if hit_sl:
                pnl = (slp - p['entry']) * p['vol'] * p['dir']
                balance += pnl
                groups[p['hour']] = groups.get(p['hour'], 0.0) + pnl
                trades.append((tm[i], pnl, 'sl'))
                positions.remove(p)
                closed_any = True
                continue
        if p['tp'] is None:
    ...
    # --- FAST-TURTLE entry: M1 renko brick double-reversal (Turtle's signal) ---
    brick_sig = 0
    if b_ao is None:
        b_ao = b_ac = float(o[i])
    while True:
        up = (b_ao if b_dir == -1 else b_ac) + BRICK * (REVERSAL if b_dir == -1 else 1)
        dn = (b_ao if b_dir == 1 else b_ac) - BRICK * (REVERSAL if b_dir == 1 else 1)
        if c[i] >= up:
            base = b_ao if b_dir == -1 else b_ac
            b_ao, b_ac, b_dir = base, base + BRICK, 1
        elif c[i] <= dn:
            base = b_ao if b_dir == 1 else b_ac
            b_ao, b_ac, b_dir = base, base - BRICK, -1
        else:
            break
        if b_pd and b_dir != b_pd:
            brick_sig = b_dir
        b_pd = b_dir
...
engine = [t for t in trades if t[2] in ('tp', 'be', 'hour_clean', 'open_end', 'sl')]
sls = [t for t in trades if t[2] == 'sl']
tps = [t for t in trades if t[2] == 'tp']
print(f"TP wins {len(tps)} | SL losses {len(sls)} (${sum(t[1] for t in sls):,.2f})")
