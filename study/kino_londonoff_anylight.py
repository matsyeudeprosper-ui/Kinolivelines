...
print(f'LONDON-OFF ANY-LIGHT: Asia+NY, yellow allowed | {NMIN}min | START {START_BAL}', flush=True)
last_flat_hour = -1
...
for i in range(N):
    ...
    if mode == -1 and not (_red(closed['h1']) and _grn(closed['m15']) and _grn(closed['m5'])):
        continue
    if mode == 0 or pending is not None or escape_active:
        continue                      # ANY light (yellow allowed)
    if cur_hour == last_entry_hour or i < 1 or i + 1 >= N:
        continue
    if 8 <= (tm[i] // 3600) % 24 < 16:
        continue                      # London hours: no new entries
    d = mode
    if d == 1:
        g_prev = c[i-1] > o[i-1]
        g_cur = c[i] > o[i]
