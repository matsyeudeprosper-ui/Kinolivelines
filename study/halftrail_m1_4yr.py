...
# --- diagnostic: does realized_profit_peak ever cross the $800 activation line? ---
def run_diag(o,h,l,c,tm,N,sigs,default_sl_pts):
    default_sl_usd = default_sl_pts*PT*SCALE
    activation_usd = 2*HT_FLOOR
    realized_peak = 0.0; realized_cum = 0.0
    bal_usd=0.0
    pending=None; in_pos=False; pos_L=None; pos_entry=None; pos_entry_t=None; pos_sl_pts=None
    max_peak_ever = 0.0
    activated_count = 0
    for j in range(N):
        if pending is not None:
            L,entry,et = pending; in_pos=True; pos_L=L; pos_entry=entry; pos_entry_t=et; pending=None
            if realized_peak >= activation_usd:
                sl_usd = max(realized_peak/2.0, HT_FLOOR); activated_count += 1
            else:
                sl_usd = default_sl_usd
            pos_sl_pts = sl_usd/(PT*SCALE)
        if j in sigs and j+1<N and not in_pos:
        ...
        if in_pos:
            ...
            if hit_tp or hit_sl:
                ...
                in_pos=False
    return max_peak_ever, activated_count

max_peak, act_count = run_diag(o,h,l,c,tm,N,sigs,SL_PTS_DEFAULT)
print(f"\nDIAGNOSTIC: highest realized_profit_peak EVER reached across all 4 years: ${max_peak:.2f}  (activation threshold: ${2*HT_FLOOR:.2f})")
print(f"trades opened while Half Trail was ACTIVE: {act_count}")
