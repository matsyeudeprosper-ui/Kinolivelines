...
print(f'TURN GATE: H1grn/M15red/M5GRN (mirror) + split + {NMIN}min | START {START_BAL}', flush=True)
last_flat_hour = -1
...
for i in range(N):
    ...
    if mode == 1 and not (_grn(closed['h1']) and _red(closed['m15']) and _grn(closed['m5'])):
        continue
    if mode == -1 and not (_red(closed['h1']) and _grn(closed['m15']) and _red(closed['m5'])):
        continue
