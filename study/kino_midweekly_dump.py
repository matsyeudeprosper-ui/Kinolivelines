...
print(f'MID-WEEKLY TRUE COSTS: toll charged both sides | START {START_BAL}', flush=True)
w1_prev = None
fib_touched = set()
fib_entered_hour = -1
fib_hour = -1
...
for i in range(N):
    ...
    if w1_prev is None:
        continue
    _w1dir = 1 if w1_prev[3] > w1_prev[0] else (-1 if w1_prev[3] < w1_prev[0] else 0)
    if _w1dir != h1dir:
        continue                      # WEEKLY candle must agree too
...
for t, pnl, kind in engine:
    mon[_dt.datetime.utcfromtimestamp(int(t)).strftime('%Y-%m')] += pnl
vals = sorted(mon.values())
print(f"BANKED max DD: ${_mdd:.2f} | months {sum(1 for v in vals if v > 0)}/{len(vals)} positive | worst ${vals[0]:.2f} | best ${vals[-1]:.2f}")

import json as _json3, datetime as _dt3
_days = {}
for t, pnl, kind in engine:
    _dk = _dt3.datetime.utcfromtimestamp(int(t)).strftime("%Y-%m-%d")
    _days[_dk] = _days.get(_dk, 0.0) + pnl
_json3.dump(_days, open("kingfisher_daily.json", "w"))
print("kingfisher daily dumped")
