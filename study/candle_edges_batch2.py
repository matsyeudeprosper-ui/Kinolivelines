...
print("building M15 bars for EMA21 (Trend Lane)...")
...
print("computing M1 EMA21 (this is the big one - 3.1M bars, causal)...")
m1_ema21 = ema(c_f, 21)

print("computing M1 ATR14 for body-size-relative-to-volatility check...")
def atr14_fast(o,h,l,c,N):
    atr = np.full(N, np.nan)
    tr = np.empty(N)
    tr[0] = h[0]-l[0]
    tr[1:] = np.maximum(h[1:]-l[1:], np.maximum(np.abs(h[1:]-c[:-1]), np.abs(l[1:]-c[:-1])))
    csum = np.cumsum(tr)
    atr[13:] = (csum[13:] - np.concatenate(([0], csum[:-14]))) / 14.0
    return atr
m1_atr = atr14_fast(o_f,h_f,l_f,c_f,N)
        ...
        if in_pos:
            ...
            if htp or hsl:
                ...
                if p1 is not None and not np.isnan(m15_ema[p1]):
                    m15_below_ema = bool(m15_c[p1] <= m15_ema[p1])

                m1_below_ema = None
                if not np.isnan(m1_ema21[pos_sig_bar]):
                    m1_below_ema = bool(c_f[pos_sig_bar] <= m1_ema21[pos_sig_bar])

                body = abs(c_f[pos_sig_bar] - o_f[pos_sig_bar])
                strong_candle = None
                if not np.isnan(m1_atr[pos_sig_bar]) and m1_atr[pos_sig_bar] > 0:
                    strong_candle = bool(body >= m1_atr[pos_sig_bar])

                trades.append(dict(side="BUY" if pos_L else "SELL", m1_fresh=m1_fresh,
                                    m15_below_ema=m15_below_ema, m1_below_ema=m1_below_ema,
                                    strong_candle=strong_candle, usd=usd, hsl=hsl))
                in_pos=False
    return trades, bal
...
def report(feature_name, key_fn):
    ...
    for k in sorted(buckets.keys(), key=str):
        w,l,usd = buckets[k]
        tot=w+l
        print(f"  {str(k):<8} n={tot:>5}  losses={l:>3}  loss%={100*l/tot if tot else 0:>6.2f}%  net=${usd:>+10,.2f}  avg/trade=${usd/tot if tot else 0:>+6.2f}")

report("M1 price below/above its own EMA21", lambda t: t['m1_below_ema'])
report("Signal candle body >= its own ATR14 (strong vs weak move)", lambda t: t['strong_candle'])

print("\n=== Trend Lane (M15 below EMA) COMBINED with M1-fresh ===")
combo = defaultdict(lambda: [0,0,0.0])
for t in trades:
    if t['m15_below_ema'] is None: continue
    key = (t['m15_below_ema'], t['m1_fresh'])
    combo[key][1 if t['hsl'] else 0] += 1
    combo[key][2] += t['usd']
print(f"{'TrendLane(below)':<18} {'M1 fresh':<10} {'n':>6} {'losses':>7} {'loss%':>7} {'net':>10}")
for k in sorted(combo.keys(), key=str):
    w,l,usd = combo[k]; tot=w+l
    print(f"{str(k[0]):<18} {str(k[1]):<10} {tot:>6} {l:>7} {100*l/tot if tot else 0:>6.2f}% {usd:>+10,.2f}")

print("\n=== BEST COMBO CHECK: TrendLane-below AND M1-fresh together vs everything else ===")
best = [t for t in trades if t['m15_below_ema'] is True and t['m1_fresh']]
rest = [t for t in trades if not (t['m15_below_ema'] is True and t['m1_fresh'])]
for label, grp in [("TrendLane+fresh (best combo)", best), ("everything else", rest)]:
    w = sum(1 for t in grp if not t['hsl']); l = sum(1 for t in grp if t['hsl'])
    usd = sum(t['usd'] for t in grp)
    tot = w+l
    print(f"  {label}: n={tot} losses={l} loss%={100*l/tot if tot else 0:.2f}% net=${usd:+,.2f} avg/trade=${usd/tot if tot else 0:+.2f}")
